...
$layout_slide2_blobview = NULL; // Initialize page object first

class clayout_slide2_blobview extends clayout {
	...
	// Page object name
	var $PageObjName = 'layout_slide2_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('layout', 'x_slide2');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('slide2');
				...
				if (trim(strval($this->Recordset->fields('slide2'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('slide2') . "\"");
				}
...
// Create page object
if (!isset($layout_slide2_blobview)) $layout_slide2_blobview = new clayout_slide2_blobview();

// Page init
$layout_slide2_blobview->Page_Init();

// Page main
$layout_slide2_blobview->Page_Main();
?>
<?php
$layout_slide2_blobview->Page_Terminate();
?>
