...
$overall_asset_view = NULL; // Initialize page object first

class coverall_asset_view extends coverall_asset {
	...
	// Page object name
	var $PageObjName = 'overall_asset_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "overall_assetlist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "overall_assetlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_overall_asset\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_overall_asset',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.foverall_assetview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($overall_asset_view)) $overall_asset_view = new coverall_asset_view();

// Page init
$overall_asset_view->Page_Init();

// Page main
$overall_asset_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$overall_asset_view->Page_Render();
?>
...
// Page object
var overall_asset_view = new ew_Page("overall_asset_view");
overall_asset_view.PageID = "view"; // Page ID
var EW_PAGE_ID = overall_asset_view.PageID; // For backward compatibility

// Form object
var foverall_assetview = new ew_Form("foverall_assetview");

// Form_CustomValidate event
foverall_assetview.Form_CustomValidate = 
...
foverall_assetview.ValidateRequired = true;
<?php } else { ?>
foverall_assetview.ValidateRequired = false; 
<?php } ?>
...
<?php $overall_asset_view->ExportOptions->Render("body") ?>
<?php if (!$overall_asset_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($overall_asset_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $overall_asset_view->ShowPageHeader(); ?>
<?php
$overall_asset_view->ShowMessage();
?>
<form name="foverall_assetview" id="foverall_assetview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="overall_asset">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_overall_assetview" class="table table-bordered table-striped">
...
<span id="el_overall_asset_uid" class="control-group">
...
<table class="ewPager">
<tr><td>
<?php if (!isset($overall_asset_view->Pager)) $overall_asset_view->Pager = new cNumericPager($overall_asset_view->StartRec, $overall_asset_view->DisplayRecs, $overall_asset_view->TotalRecs, $overall_asset_view->RecRange) ?>
<?php if ($overall_asset_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($overall_asset_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $overall_asset_view->PageUrl() ?>start=<?php echo $overall_asset_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($overall_asset_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $overall_asset_view->PageUrl() ?>start=<?php echo $overall_asset_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($overall_asset_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $overall_asset_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($overall_asset_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $overall_asset_view->PageUrl() ?>start=<?php echo $overall_asset_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($overall_asset_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $overall_asset_view->PageUrl() ?>start=<?php echo $overall_asset_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<script type="text/javascript">
foverall_assetview.Init();
</script>
<?php
$overall_asset_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($overall_asset->Export == "") { ?>
...
<?php
$overall_asset_view->Page_Terminate();
?>
