...
$witness_tb_view = NULL; // Initialize page object first

class cwitness_tb_view extends cwitness_tb {
	...
	// Page object name
	var $PageObjName = 'witness_tb_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("witness_tblist.php"); // Return to list page
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "witness_tblist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "witness_tblist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_witness_tb\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_witness_tb',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fwitness_tbview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($witness_tb_view)) $witness_tb_view = new cwitness_tb_view();

// Page init
$witness_tb_view->Page_Init();

// Page main
$witness_tb_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$witness_tb_view->Page_Render();
?>
...
// Page object
var witness_tb_view = new ew_Page("witness_tb_view");
witness_tb_view.PageID = "view"; // Page ID
var EW_PAGE_ID = witness_tb_view.PageID; // For backward compatibility

// Form object
var fwitness_tbview = new ew_Form("fwitness_tbview");

// Form_CustomValidate event
fwitness_tbview.Form_CustomValidate = 
...
fwitness_tbview.ValidateRequired = true;
<?php } else { ?>
fwitness_tbview.ValidateRequired = false; 
<?php } ?>
...
<?php $witness_tb_view->ExportOptions->Render("body") ?>
<?php if (!$witness_tb_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($witness_tb_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $witness_tb_view->ShowPageHeader(); ?>
<?php
$witness_tb_view->ShowMessage();
?>
<form name="fwitness_tbview" id="fwitness_tbview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="witness_tb">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_witness_tbview" class="table table-bordered table-striped">
<?php if ($witness_tb->id->Visible) { // id ?>
	<tr id="r_id">
		<td><span id="elh_witness_tb_id"><?php echo $witness_tb->id->FldCaption() ?></span></td>
		<td<?php echo $witness_tb->id->CellAttributes() ?>>
<span id="el_witness_tb_id" class="control-group">
...
<span id="el_witness_tb_uid" class="control-group">
...
<?php if ($witness_tb->title->Visible) { // title ?>
	<tr id="r_title">
		<td><span id="elh_witness_tb_title"><?php echo $witness_tb->title->FldCaption() ?></span></td>
...
<?php if ($witness_tb->rtionship->Visible) { // rtionship ?>
	<tr id="r_rtionship">
		<td><span id="elh_witness_tb_rtionship"><?php echo $witness_tb->rtionship->FldCaption() ?></span></td>
...
<?php if ($witness_tb->_email->Visible) { // email ?>
	<tr id="r__email">
		<td><span id="elh_witness_tb__email"><?php echo $witness_tb->_email->FldCaption() ?></span></td>
...
<?php if ($witness_tb->phone->Visible) { // phone ?>
	<tr id="r_phone">
		<td><span id="elh_witness_tb_phone"><?php echo $witness_tb->phone->FldCaption() ?></span></td>
...
<?php if ($witness_tb->addr->Visible) { // addr ?>
	<tr id="r_addr">
		<td><span id="elh_witness_tb_addr"><?php echo $witness_tb->addr->FldCaption() ?></span></td>
		<td<?php echo $witness_tb->addr->CellAttributes() ?>>
<span id="el_witness_tb_addr" class="control-group">
<span<?php echo $witness_tb->addr->ViewAttributes() ?>>
<?php echo $witness_tb->addr->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($witness_tb->city->Visible) { // city ?>
	<tr id="r_city">
		<td><span id="elh_witness_tb_city"><?php echo $witness_tb->city->FldCaption() ?></span></td>
...
<?php if ($witness_tb->state->Visible) { // state ?>
	<tr id="r_state">
		<td><span id="elh_witness_tb_state"><?php echo $witness_tb->state->FldCaption() ?></span></td>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($witness_tb_view->Pager)) $witness_tb_view->Pager = new cNumericPager($witness_tb_view->StartRec, $witness_tb_view->DisplayRecs, $witness_tb_view->TotalRecs, $witness_tb_view->RecRange) ?>
<?php if ($witness_tb_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($witness_tb_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $witness_tb_view->PageUrl() ?>start=<?php echo $witness_tb_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($witness_tb_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $witness_tb_view->PageUrl() ?>start=<?php echo $witness_tb_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($witness_tb_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $witness_tb_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($witness_tb_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $witness_tb_view->PageUrl() ?>start=<?php echo $witness_tb_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($witness_tb_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $witness_tb_view->PageUrl() ?>start=<?php echo $witness_tb_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<script type="text/javascript">
fwitness_tbview.Init();
</script>
<?php
$witness_tb_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($witness_tb->Export == "") { ?>
...
<?php
$witness_tb_view->Page_Terminate();
?>
