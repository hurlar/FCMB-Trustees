...
$divorce_tb_view = NULL; // Initialize page object first

class cdivorce_tb_view extends cdivorce_tb {
	...
	// Page object name
	var $PageObjName = 'divorce_tb_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "divorce_tblist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "divorce_tblist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_divorce_tb\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_divorce_tb',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fdivorce_tbview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($divorce_tb_view)) $divorce_tb_view = new cdivorce_tb_view();

// Page init
$divorce_tb_view->Page_Init();

// Page main
$divorce_tb_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$divorce_tb_view->Page_Render();
?>
...
// Page object
var divorce_tb_view = new ew_Page("divorce_tb_view");
divorce_tb_view.PageID = "view"; // Page ID
var EW_PAGE_ID = divorce_tb_view.PageID; // For backward compatibility

// Form object
var fdivorce_tbview = new ew_Form("fdivorce_tbview");

// Form_CustomValidate event
fdivorce_tbview.Form_CustomValidate = 
...
fdivorce_tbview.ValidateRequired = true;
<?php } else { ?>
fdivorce_tbview.ValidateRequired = false; 
<?php } ?>
...
<?php $divorce_tb_view->ExportOptions->Render("body") ?>
<?php if (!$divorce_tb_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($divorce_tb_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $divorce_tb_view->ShowPageHeader(); ?>
<?php
$divorce_tb_view->ShowMessage();
?>
<form name="fdivorce_tbview" id="fdivorce_tbview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="divorce_tb">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_divorce_tbview" class="table table-bordered table-striped">
...
<span id="el_divorce_tb_uid" class="control-group">
...
<table class="ewPager">
<tr><td>
<?php if (!isset($divorce_tb_view->Pager)) $divorce_tb_view->Pager = new cNumericPager($divorce_tb_view->StartRec, $divorce_tb_view->DisplayRecs, $divorce_tb_view->TotalRecs, $divorce_tb_view->RecRange) ?>
<?php if ($divorce_tb_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($divorce_tb_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $divorce_tb_view->PageUrl() ?>start=<?php echo $divorce_tb_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($divorce_tb_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $divorce_tb_view->PageUrl() ?>start=<?php echo $divorce_tb_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($divorce_tb_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $divorce_tb_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($divorce_tb_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $divorce_tb_view->PageUrl() ?>start=<?php echo $divorce_tb_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($divorce_tb_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $divorce_tb_view->PageUrl() ?>start=<?php echo $divorce_tb_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<script type="text/javascript">
fdivorce_tbview.Init();
</script>
<?php
$divorce_tb_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($divorce_tb->Export == "") { ?>
...
<?php
$divorce_tb_view->Page_Terminate();
?>
