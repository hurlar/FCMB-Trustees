...
$will_tb_pdflink_blobview = NULL; // Initialize page object first

class cwill_tb_pdflink_blobview extends cwill_tb {
	...
	// Page object name
	var $PageObjName = 'will_tb_pdflink_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('will_tb', 'x_pdflink');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('pdflink');
				...
				if (trim(strval($this->Recordset->fields('pdflink'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('pdflink') . "\"");
				}
...
// Create page object
if (!isset($will_tb_pdflink_blobview)) $will_tb_pdflink_blobview = new cwill_tb_pdflink_blobview();

// Page init
$will_tb_pdflink_blobview->Page_Init();

// Page main
$will_tb_pdflink_blobview->Page_Main();
?>
<?php
$will_tb_pdflink_blobview->Page_Terminate();
?>
