...
$overall_asset_delete = NULL; // Initialize page object first

class coverall_asset_delete extends coverall_asset {
	...
	// Page object name
	var $PageObjName = 'overall_asset_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("overall_assetlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in overall_asset class, overall_assetinfo.php
...
// Create page object
if (!isset($overall_asset_delete)) $overall_asset_delete = new coverall_asset_delete();

// Page init
$overall_asset_delete->Page_Init();

// Page main
$overall_asset_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$overall_asset_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var overall_asset_delete = new ew_Page("overall_asset_delete");
overall_asset_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = overall_asset_delete.PageID; // For backward compatibility

// Form object
var foverall_assetdelete = new ew_Form("foverall_assetdelete");

// Form_CustomValidate event
foverall_assetdelete.Form_CustomValidate = 
...
foverall_assetdelete.ValidateRequired = true;
<?php } else { ?>
foverall_assetdelete.ValidateRequired = false; 
<?php } ?>
...
if ($overall_asset_delete->Recordset = $overall_asset_delete->LoadRecordset())
	$overall_asset_deleteTotalRecs = $overall_asset_delete->Recordset->RecordCount(); // Get record count
if ($overall_asset_deleteTotalRecs <= 0) { // No record found, exit
	if ($overall_asset_delete->Recordset)
		$overall_asset_delete->Recordset->Close();
	$overall_asset_delete->Page_Terminate("overall_assetlist.php"); // Return to list
}
?>
<?php $Breadcrumb->Render(); ?>
<?php $overall_asset_delete->ShowPageHeader(); ?>
<?php
$overall_asset_delete->ShowMessage();
?>
<form name="foverall_assetdelete" id="foverall_assetdelete" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="overall_asset">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($overall_asset_delete->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_overall_assetdelete" class="ewTable ewTableSeparate">
<?php echo $overall_asset->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
<?php if ($overall_asset->id->Visible) { // id ?>
		<td><span id="elh_overall_asset_id" class="overall_asset_id"><?php echo $overall_asset->id->FldCaption() ?></span></td>
<?php } ?>
<?php if ($overall_asset->beneficiaryid->Visible) { // beneficiaryid ?>
		<td><span id="elh_overall_asset_beneficiaryid" class="overall_asset_beneficiaryid"><?php echo $overall_asset->beneficiaryid->FldCaption() ?></span></td>
<?php } ?>
<?php if ($overall_asset->propertyid->Visible) { // propertyid ?>
		<td><span id="elh_overall_asset_propertyid" class="overall_asset_propertyid"><?php echo $overall_asset->propertyid->FldCaption() ?></span></td>
<?php } ?>
<?php if ($overall_asset->property_type->Visible) { // property_type ?>
		<td><span id="elh_overall_asset_property_type" class="overall_asset_property_type"><?php echo $overall_asset->property_type->FldCaption() ?></span></td>
<?php } ?>
<?php if ($overall_asset->percentage->Visible) { // percentage ?>
		<td><span id="elh_overall_asset_percentage" class="overall_asset_percentage"><?php echo $overall_asset->percentage->FldCaption() ?></span></td>
<?php } ?>
<?php if ($overall_asset->datecreated->Visible) { // datecreated ?>
		<td><span id="elh_overall_asset_datecreated" class="overall_asset_datecreated"><?php echo $overall_asset->datecreated->FldCaption() ?></span></td>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$overall_asset_delete->RecCnt = 0;
$i = 0;
while (!$overall_asset_delete->Recordset->EOF) {
	$overall_asset_delete->RecCnt++;
	$overall_asset_delete->RowCnt++;

	// Set row properties
	$overall_asset->ResetAttrs();
	$overall_asset->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$overall_asset_delete->LoadRowValues($overall_asset_delete->Recordset);

	// Render row
	$overall_asset_delete->RenderRow();
?>
	<tr<?php echo $overall_asset->RowAttributes() ?>>
<?php if ($overall_asset->id->Visible) { // id ?>
		<td<?php echo $overall_asset->id->CellAttributes() ?>>
<span id="el<?php echo $overall_asset_delete->RowCnt ?>_overall_asset_id" class="control-group overall_asset_id">
...
<span id="el<?php echo $overall_asset_delete->RowCnt ?>_overall_asset_beneficiaryid" class="control-group overall_asset_beneficiaryid">
...
<span id="el<?php echo $overall_asset_delete->RowCnt ?>_overall_asset_propertyid" class="control-group overall_asset_propertyid">
...
<span id="el<?php echo $overall_asset_delete->RowCnt ?>_overall_asset_property_type" class="control-group overall_asset_property_type">
...
<span id="el<?php echo $overall_asset_delete->RowCnt ?>_overall_asset_percentage" class="control-group overall_asset_percentage">
...
<span id="el<?php echo $overall_asset_delete->RowCnt ?>_overall_asset_datecreated" class="control-group overall_asset_datecreated">
...
<?php
	$overall_asset_delete->Recordset->MoveNext();
}
$overall_asset_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
foverall_assetdelete.Init();
</script>
<?php
$overall_asset_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$overall_asset_delete->Page_Terminate();
?>
