...
class corders_view extends corders {
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_orders\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_orders',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fordersview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
fordersview.ValidateRequired = true;
<?php } else { ?>
fordersview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fordersview.Lists["x_customerid"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_name","x_lname","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php if (!$orders_view->ExportOptions->UseDropDownButton) { ?>
</div>
...
?>
<form name="fordersview" id="fordersview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="orders">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_ordersview" class="table table-bordered table-striped">
...
<?php if ($orders->customerid->Visible) { // customerid ?>
	<tr id="r_customerid">
		<td><span id="elh_orders_customerid"><?php echo $orders->customerid->FldCaption() ?></span></td>
...
<?php echo $orders->ship->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $orders->total->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($orders->status->Visible) { // status ?>
	<tr id="r_status">
		<td><span id="elh_orders_status"><?php echo $orders->status->FldCaption() ?></span></td>
...
<?php if ($orders->date->Visible) { // date ?>
	<tr id="r_date">
		<td><span id="elh_orders_date"><?php echo $orders->date->FldCaption() ?></span></td>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($orders_view->Pager)) $orders_view->Pager = new cNumericPager($orders_view->StartRec, $orders_view->DisplayRecs, $orders_view->TotalRecs, $orders_view->RecRange) ?>
<?php if ($orders_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($orders_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $orders_view->PageUrl() ?>start=<?php echo $orders_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($orders_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $orders_view->PageUrl() ?>start=<?php echo $orders_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($orders_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $orders_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($orders_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $orders_view->PageUrl() ?>start=<?php echo $orders_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($orders_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $orders_view->PageUrl() ?>start=<?php echo $orders_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
