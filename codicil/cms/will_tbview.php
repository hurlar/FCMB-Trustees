...
$will_tb_view = NULL; // Initialize page object first

class cwill_tb_view extends cwill_tb {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "will_tblist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "will_tblist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_will_tb\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_will_tb',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fwill_tbview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($will_tb_view)) $will_tb_view = new cwill_tb_view();

// Page init
$will_tb_view->Page_Init();

// Page main
$will_tb_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$will_tb_view->Page_Render();
?>
...
// Page object
var will_tb_view = new ew_Page("will_tb_view");
will_tb_view.PageID = "view"; // Page ID
var EW_PAGE_ID = will_tb_view.PageID; // For backward compatibility

// Form object
var fwill_tbview = new ew_Form("fwill_tbview");

// Form_CustomValidate event
fwill_tbview.Form_CustomValidate = 
...
fwill_tbview.ValidateRequired = true;
<?php } else { ?>
fwill_tbview.ValidateRequired = false; 
<?php } ?>
...
<?php $will_tb_view->ExportOptions->Render("body") ?>
<?php if (!$will_tb_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($will_tb_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $will_tb_view->ShowPageHeader(); ?>
<?php
$will_tb_view->ShowMessage();
?>
<form name="fwill_tbview" id="fwill_tbview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="will_tb">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_will_tbview" class="table table-bordered table-striped">
...
<?php echo $will_tb->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $will_tb->name->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<a<?php echo $will_tb->pdflink->LinkAttributes() ?>><?php echo $will_tb->pdflink->ViewValue ?></a>
<?php } elseif (!in_array($will_tb->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!empty($will_tb->pdflink->Upload->DbValue)) { ?>
<?php echo $will_tb->pdflink->ViewValue ?>
<?php } elseif (!in_array($will_tb->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } ?>
...
<?php echo $will_tb->url_link->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if (!ew_EmptyStr($will_tb->rating_order->ViewValue) && $will_tb->rating_order->LinkAttributes() <> "") { ?>
<a<?php echo $will_tb->rating_order->LinkAttributes() ?>><?php echo $will_tb->rating_order->ViewValue ?></a>
<?php } else { ?>
<?php echo $will_tb->rating_order->ViewValue ?>
<?php } ?>
</span>
</span>
</td>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($will_tb_view->Pager)) $will_tb_view->Pager = new cNumericPager($will_tb_view->StartRec, $will_tb_view->DisplayRecs, $will_tb_view->TotalRecs, $will_tb_view->RecRange) ?>
<?php if ($will_tb_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($will_tb_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $will_tb_view->PageUrl() ?>start=<?php echo $will_tb_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($will_tb_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $will_tb_view->PageUrl() ?>start=<?php echo $will_tb_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($will_tb_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $will_tb_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($will_tb_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $will_tb_view->PageUrl() ?>start=<?php echo $will_tb_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($will_tb_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $will_tb_view->PageUrl() ?>start=<?php echo $will_tb_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
