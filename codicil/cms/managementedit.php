...
// Page object
var management_edit = new ew_Page("management_edit");
management_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = management_edit.PageID; // For backward compatibility

// Form object
var fmanagementedit = new ew_Form("fmanagementedit");
...
fmanagementedit.ValidateRequired = true;
<?php } else { ?>
fmanagementedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmanagementedit.Lists["x_category"] = {"LinkField":"x_category","Ajax":null,"AutoFill":false,"DisplayFields":["x_category","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
?>
<form name="fmanagementedit" id="fmanagementedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="management">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_managementedit" class="table table-bordered table-striped">
...
<?php echo $management->id->EditValue ?></span>
</span>
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($management->id->CurrentValue) ?>">
...
<script type="text/javascript">
ew_CreateEditor("fmanagementedit", "x_descrp", 35, 4, <?php echo ($management->descrp->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<script type="text/javascript">
fmanagementedit.Lists["x_category"].Options = <?php echo (is_array($management->category->EditValue)) ? ew_ArrayToJson($management->category->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($management_edit->Pager)) $management_edit->Pager = new cNumericPager($management_edit->StartRec, $management_edit->DisplayRecs, $management_edit->TotalRecs, $management_edit->RecRange) ?>
<?php if ($management_edit->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($management_edit->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $management_edit->PageUrl() ?>start=<?php echo $management_edit->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($management_edit->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $management_edit->PageUrl() ?>start=<?php echo $management_edit->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($management_edit->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $management_edit->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($management_edit->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $management_edit->PageUrl() ?>start=<?php echo $management_edit->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($management_edit->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $management_edit->PageUrl() ?>start=<?php echo $management_edit->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
