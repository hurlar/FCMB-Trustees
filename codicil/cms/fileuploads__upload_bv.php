...
$fileuploads__upload_blobview = NULL; // Initialize page object first

class cfileuploads__upload_blobview extends cfileuploads {
	...
	// Page object name
	var $PageObjName = 'fileuploads__upload_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('fileuploads', 'x__upload');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('upload');
				...
				if (trim(strval($this->Recordset->fields('upload'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('upload') . "\"");
				}
...
// Create page object
if (!isset($fileuploads__upload_blobview)) $fileuploads__upload_blobview = new cfileuploads__upload_blobview();

// Page init
$fileuploads__upload_blobview->Page_Init();

// Page main
$fileuploads__upload_blobview->Page_Main();
?>
<?php
$fileuploads__upload_blobview->Page_Terminate();
?>
