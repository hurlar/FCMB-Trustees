...
$news_tb_view = NULL; // Initialize page object first

class cnews_tb_view extends cnews_tb {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "news_tblist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "news_tblist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_news_tb\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_news_tb',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fnews_tbview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($news_tb_view)) $news_tb_view = new cnews_tb_view();

// Page init
$news_tb_view->Page_Init();

// Page main
$news_tb_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$news_tb_view->Page_Render();
?>
...
// Page object
var news_tb_view = new ew_Page("news_tb_view");
news_tb_view.PageID = "view"; // Page ID
var EW_PAGE_ID = news_tb_view.PageID; // For backward compatibility

// Form object
var fnews_tbview = new ew_Form("fnews_tbview");

// Form_CustomValidate event
fnews_tbview.Form_CustomValidate = 
...
fnews_tbview.ValidateRequired = true;
<?php } else { ?>
fnews_tbview.ValidateRequired = false; 
<?php } ?>
...
<?php $news_tb_view->ExportOptions->Render("body") ?>
<?php if (!$news_tb_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($news_tb_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $news_tb_view->ShowPageHeader(); ?>
<?php
$news_tb_view->ShowMessage();
?>
<form name="fnews_tbview" id="fnews_tbview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="news_tb">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_news_tbview" class="table table-bordered table-striped">
...
<?php echo $news_tb->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($news_tb->title->Visible) { // title ?>
	<tr id="r_title">
		<td><span id="elh_news_tb_title"><?php echo $news_tb->title->FldCaption() ?></span></td>
...
<?php echo $news_tb->title->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<span<?php echo $news_tb->content->ViewAttributes() ?>>
<?php echo $news_tb->content->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $news_tb->postdate->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<span<?php echo $news_tb->img->ViewAttributes() ?>>
<?php if ($news_tb->img->LinkAttributes() <> "") { ?>
<?php if (!empty($news_tb->img->Upload->DbValue)) { ?>
<?php echo $news_tb->img->ViewValue ?>
<?php } elseif (!in_array($news_tb->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!empty($news_tb->img->Upload->DbValue)) { ?>
...
<?php if ($news_tb->rate_ord->Visible) { // rate_ord ?>
	<tr id="r_rate_ord">
		<td><span id="elh_news_tb_rate_ord"><?php echo $news_tb->rate_ord->FldCaption() ?></span></td>
		<td<?php echo $news_tb->rate_ord->CellAttributes() ?>>
<span id="el_news_tb_rate_ord" class="control-group">
<span<?php echo $news_tb->rate_ord->ViewAttributes() ?>>
<?php echo $news_tb->rate_ord->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $news_tb->venue->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $news_tb->time->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($news_tb_view->Pager)) $news_tb_view->Pager = new cNumericPager($news_tb_view->StartRec, $news_tb_view->DisplayRecs, $news_tb_view->TotalRecs, $news_tb_view->RecRange) ?>
<?php if ($news_tb_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($news_tb_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $news_tb_view->PageUrl() ?>start=<?php echo $news_tb_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($news_tb_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $news_tb_view->PageUrl() ?>start=<?php echo $news_tb_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($news_tb_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $news_tb_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($news_tb_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $news_tb_view->PageUrl() ?>start=<?php echo $news_tb_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($news_tb_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $news_tb_view->PageUrl() ?>start=<?php echo $news_tb_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
