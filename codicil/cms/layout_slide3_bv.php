...
$layout_slide3_blobview = NULL; // Initialize page object first

class clayout_slide3_blobview extends clayout {
	...
	// Page object name
	var $PageObjName = 'layout_slide3_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('layout', 'x_slide3');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('slide3');
				...
				if (trim(strval($this->Recordset->fields('slide3'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('slide3') . "\"");
				}
...
// Create page object
if (!isset($layout_slide3_blobview)) $layout_slide3_blobview = new clayout_slide3_blobview();

// Page init
$layout_slide3_blobview->Page_Init();

// Page main
$layout_slide3_blobview->Page_Main();
?>
<?php
$layout_slide3_blobview->Page_Terminate();
?>
