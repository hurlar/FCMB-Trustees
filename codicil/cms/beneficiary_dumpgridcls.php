...
$beneficiary_dump_grid = NULL; // Initialize page object first

class cbeneficiary_dump_grid extends cbeneficiary_dump {
	...
	// Page object name
	var $PageObjName = 'beneficiary_dump_grid';

	// Grid form hidden field names
	var $FormName = 'fbeneficiary_dumpgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["beneficiary_dump"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_childid") && $objForm->HasValue("o_childid") && $this->childid->CurrentValue <> $this->childid->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_title") && $objForm->HasValue("o_title") && $this->title->CurrentValue <> $this->title->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_fullname") && $objForm->HasValue("o_fullname") && $this->fullname->CurrentValue <> $this->fullname->OldValue)
			return FALSE;
	...
	function LoadFormValues() {
		...
		$this->childid->setOldValue($objForm->GetValue("o_childid"));
		...
		$this->fullname->setOldValue($objForm->GetValue("o_fullname"));
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "children_details") {
			...
			if ($GLOBALS["children_details"]->EventCancelled) $this->EventCancelled = TRUE;
		}
