...
$nextofkin_view = NULL; // Initialize page object first

class cnextofkin_view extends cnextofkin {
	...
	// Page object name
	var $PageObjName = 'nextofkin_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "nextofkinlist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "nextofkinlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_nextofkin\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_nextofkin',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fnextofkinview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($nextofkin_view)) $nextofkin_view = new cnextofkin_view();

// Page init
$nextofkin_view->Page_Init();

// Page main
$nextofkin_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$nextofkin_view->Page_Render();
?>
...
// Page object
var nextofkin_view = new ew_Page("nextofkin_view");
nextofkin_view.PageID = "view"; // Page ID
var EW_PAGE_ID = nextofkin_view.PageID; // For backward compatibility

// Form object
var fnextofkinview = new ew_Form("fnextofkinview");

// Form_CustomValidate event
fnextofkinview.Form_CustomValidate = 
...
fnextofkinview.ValidateRequired = true;
<?php } else { ?>
fnextofkinview.ValidateRequired = false; 
<?php } ?>
...
<?php $nextofkin_view->ExportOptions->Render("body") ?>
<?php if (!$nextofkin_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($nextofkin_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $nextofkin_view->ShowPageHeader(); ?>
<?php
$nextofkin_view->ShowMessage();
?>
<form name="fnextofkinview" id="fnextofkinview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="nextofkin">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_nextofkinview" class="table table-bordered table-striped">
...
<?php echo $nextofkin->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($nextofkin->uid->Visible) { // uid ?>
	<tr id="r_uid">
		<td><span id="elh_nextofkin_uid"><?php echo $nextofkin->uid->FldCaption() ?></span></td>
...
<span<?php echo $nextofkin->address->ViewAttributes() ?>>
<?php echo $nextofkin->address->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($nextofkin_view->Pager)) $nextofkin_view->Pager = new cNumericPager($nextofkin_view->StartRec, $nextofkin_view->DisplayRecs, $nextofkin_view->TotalRecs, $nextofkin_view->RecRange) ?>
<?php if ($nextofkin_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($nextofkin_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $nextofkin_view->PageUrl() ?>start=<?php echo $nextofkin_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($nextofkin_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $nextofkin_view->PageUrl() ?>start=<?php echo $nextofkin_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($nextofkin_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $nextofkin_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($nextofkin_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $nextofkin_view->PageUrl() ?>start=<?php echo $nextofkin_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($nextofkin_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $nextofkin_view->PageUrl() ?>start=<?php echo $nextofkin_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
