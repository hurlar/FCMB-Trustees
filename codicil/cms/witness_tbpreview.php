...
$witness_tb_preview = NULL; // Initialize page object first

class cwitness_tb_preview extends cwitness_tb {
	...
	// Page object name
	var $PageObjName = 'witness_tb_preview';
...
// Create page object
if (!isset($witness_tb_preview)) $witness_tb_preview = new cwitness_tb_preview();

// Page init
$witness_tb_preview->Page_Init();

// Page main
$witness_tb_preview->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$witness_tb_preview->Page_Render();
?>
<?php $witness_tb_preview->ShowPageHeader(); ?>
<?php if ($witness_tb_preview->TotalRecs > 0) { ?>
...
<?php if ($witness_tb->id->Visible) { // id ?>
			<td><?php echo $witness_tb->id->FldCaption() ?></td>
<?php } ?>
<?php if ($witness_tb->uid->Visible) { // uid ?>
			<td><?php echo $witness_tb->uid->FldCaption() ?></td>
<?php } ?>
<?php if ($witness_tb->title->Visible) { // title ?>
			<td><?php echo $witness_tb->title->FldCaption() ?></td>
<?php } ?>
<?php if ($witness_tb->fullname->Visible) { // fullname ?>
			<td><?php echo $witness_tb->fullname->FldCaption() ?></td>
<?php } ?>
<?php if ($witness_tb->rtionship->Visible) { // rtionship ?>
			<td><?php echo $witness_tb->rtionship->FldCaption() ?></td>
<?php } ?>
<?php if ($witness_tb->_email->Visible) { // email ?>
			<td><?php echo $witness_tb->_email->FldCaption() ?></td>
<?php } ?>
<?php if ($witness_tb->phone->Visible) { // phone ?>
			<td><?php echo $witness_tb->phone->FldCaption() ?></td>
<?php } ?>
<?php if ($witness_tb->city->Visible) { // city ?>
			<td><?php echo $witness_tb->city->FldCaption() ?></td>
<?php } ?>
<?php if ($witness_tb->state->Visible) { // state ?>
			<td><?php echo $witness_tb->state->FldCaption() ?></td>
<?php } ?>
<?php if ($witness_tb->datecreated->Visible) { // datecreated ?>
			<td><?php echo $witness_tb->datecreated->FldCaption() ?></td>
<?php } ?>
		</tr>
	</thead>
	<tbody><!-- Table body -->
<?php
$witness_tb_preview->RecCount = 0;
$witness_tb_preview->RowCnt = 0;
while ($witness_tb_preview->Recordset && !$witness_tb_preview->Recordset->EOF) {

	// Init row class and style
	$witness_tb_preview->RecCount++;
	$witness_tb_preview->RowCnt++;
	$witness_tb->CssClass = "";
	$witness_tb->CssStyle = "";
	$witness_tb->LoadListRowValues($witness_tb_preview->Recordset);

	// Render row
	$witness_tb->RowType = EW_ROWTYPE_PREVIEW; // Preview record
	$witness_tb->RenderListRow();
?>
...
<?php
	$witness_tb_preview->Recordset->MoveNext();
}
?>
	</tbody>
</table>
...
<?php if ($witness_tb_preview->TotalRecs > 0) { ?>
<div class="ewDetailCount">(<?php echo $witness_tb_preview->TotalRecs ?>&nbsp;<?php echo $Language->Phrase("Record") ?>)</div>
<?php } ?>
<?php
$witness_tb_preview->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php
	if ($witness_tb_preview->Recordset)
		$witness_tb_preview->Recordset->Close();
} else { ?>
...
?>
<?php
$witness_tb_preview->Page_Terminate();
?>
