...
$addinfo_tb_view = NULL; // Initialize page object first

class caddinfo_tb_view extends caddinfo_tb {
	...
	// Page object name
	var $PageObjName = 'addinfo_tb_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "addinfo_tblist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "addinfo_tblist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_addinfo_tb\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_addinfo_tb',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.faddinfo_tbview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($addinfo_tb_view)) $addinfo_tb_view = new caddinfo_tb_view();

// Page init
$addinfo_tb_view->Page_Init();

// Page main
$addinfo_tb_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$addinfo_tb_view->Page_Render();
?>
...
// Page object
var addinfo_tb_view = new ew_Page("addinfo_tb_view");
addinfo_tb_view.PageID = "view"; // Page ID
var EW_PAGE_ID = addinfo_tb_view.PageID; // For backward compatibility

// Form object
var faddinfo_tbview = new ew_Form("faddinfo_tbview");

// Form_CustomValidate event
faddinfo_tbview.Form_CustomValidate = 
...
faddinfo_tbview.ValidateRequired = true;
<?php } else { ?>
faddinfo_tbview.ValidateRequired = false; 
<?php } ?>
...
<?php $addinfo_tb_view->ExportOptions->Render("body") ?>
<?php if (!$addinfo_tb_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($addinfo_tb_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $addinfo_tb_view->ShowPageHeader(); ?>
<?php
$addinfo_tb_view->ShowMessage();
?>
<form name="faddinfo_tbview" id="faddinfo_tbview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="addinfo_tb">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_addinfo_tbview" class="table table-bordered table-striped">
...
<span id="el_addinfo_tb_uid" class="control-group">
...
<span<?php echo $addinfo_tb->spinstruct->ViewAttributes() ?>>
<?php echo $addinfo_tb->spinstruct->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($addinfo_tb_view->Pager)) $addinfo_tb_view->Pager = new cNumericPager($addinfo_tb_view->StartRec, $addinfo_tb_view->DisplayRecs, $addinfo_tb_view->TotalRecs, $addinfo_tb_view->RecRange) ?>
<?php if ($addinfo_tb_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($addinfo_tb_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $addinfo_tb_view->PageUrl() ?>start=<?php echo $addinfo_tb_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($addinfo_tb_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $addinfo_tb_view->PageUrl() ?>start=<?php echo $addinfo_tb_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($addinfo_tb_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $addinfo_tb_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($addinfo_tb_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $addinfo_tb_view->PageUrl() ?>start=<?php echo $addinfo_tb_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($addinfo_tb_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $addinfo_tb_view->PageUrl() ?>start=<?php echo $addinfo_tb_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<script type="text/javascript">
faddinfo_tbview.Init();
</script>
<?php
$addinfo_tb_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($addinfo_tb->Export == "") { ?>
...
<?php
$addinfo_tb_view->Page_Terminate();
?>
