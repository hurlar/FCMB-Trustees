...
$spouse_tb_list = NULL; // Initialize page object first

class cspouse_tb_list extends cspouse_tb {
	...
	// Page object name
	var $PageObjName = 'spouse_tb_list';

	// Grid form hidden field names
	var $FormName = 'fspouse_tblist';
	...
	function __construct() {
		...
		$this->AddUrl = "spouse_tbadd.php?" . EW_TABLE_SHOW_DETAIL . "=";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "spouse_tbdelete.php";
		$this->MultiUpdateUrl = "spouse_tbupdate.php";
	...
	function BasicSearchSQL($Keyword) {
		...
		$this->BuildBasicSearchSQL($sWhere, $this->altphoneno, $Keyword);
		...
		$this->BuildBasicSearchSQL($sWhere, $this->citym, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $this->countrym, $Keyword);
		return $sWhere;
	}
	...
	function RenderListOptions() {
		...
		if ($Security->IsLoggedIn()) {
			...
			$body = "<a class=\"btn btn-small ewRowLink ewDetailList\" data-action=\"list\" href=\"" . ew_HtmlEncode("children_detailslist.php?" . EW_TABLE_SHOW_MASTER . "=spouse_tb&uid=" . strval($this->uid->CurrentValue) . "") . "\">" . $body . "</a>";
			$links = "";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewAction ewMultiDelete\" href=\"\" onclick=\"ew_SubmitSelected(document.fspouse_tblist, '" . $this->MultiDeleteUrl . "', ewLanguage.Phrase('DeleteMultiConfirmMsg'));return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = ($Security->IsLoggedIn());
	...
	function RenderOtherOptions() {
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->CustomActions as $action => $name) {
				...
				$item->Body = "<a class=\"ewAction ewCustomAction\" href=\"\" onclick=\"ew_SubmitSelected(document.fspouse_tblist, '" . ew_CurrentUrl() . "', null, '" . $action . "');return false;\">" . $name . "</a>";
			}
	...
	function RenderListOptionsExt() {
		...
		if ($Security->IsLoggedIn()) {			
			...
			$links .= $link;
			$detaillnk = ew_JsEncode3("children_detailslist.php?" . EW_TABLE_SHOW_MASTER . "=spouse_tb&uid=" . urlencode(strval($this->uid->CurrentValue)) . "");
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_spouse_tb\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_spouse_tb',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fspouse_tblist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($spouse_tb_list)) $spouse_tb_list = new cspouse_tb_list();

// Page init
$spouse_tb_list->Page_Init();

// Page main
$spouse_tb_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$spouse_tb_list->Page_Render();
?>
...
// Page object
var spouse_tb_list = new ew_Page("spouse_tb_list");
spouse_tb_list.PageID = "list"; // Page ID
var EW_PAGE_ID = spouse_tb_list.PageID; // For backward compatibility

// Form object
var fspouse_tblist = new ew_Form("fspouse_tblist");
fspouse_tblist.FormKeyCountName = '<?php echo $spouse_tb_list->FormKeyCountName ?>';

// Form_CustomValidate event
fspouse_tblist.Form_CustomValidate = 
...
fspouse_tblist.ValidateRequired = true;
<?php } else { ?>
fspouse_tblist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
// Form object for search

var fspouse_tblistsrch = new ew_Form("fspouse_tblistsrch");
</script>
<style type="text/css">
...
<?php } ?>
<?php if ($spouse_tb->getCurrentMasterTable() == "" && $spouse_tb_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $spouse_tb_list->ExportOptions->Render("body") ?></div>
<?php } ?>
<?php if (($spouse_tb->Export == "") || (EW_EXPORT_MASTER_RECORD && $spouse_tb->Export == "print")) { ?>
<?php
$gsMasterReturnUrl = "comprehensivewill_tblist.php";
if ($spouse_tb_list->DbMasterFilter <> "" && $spouse_tb->getCurrentMasterTable() == "comprehensivewill_tb") {
	if ($spouse_tb_list->MasterRecordExists) {
		if ($spouse_tb->getCurrentMasterTable() == $spouse_tb->TableVar) $gsMasterReturnUrl .= "?" . EW_TABLE_SHOW_MASTER . "=";
?>
<?php if ($spouse_tb_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $spouse_tb_list->ExportOptions->Render("body") ?></div>
<?php } ?>
...
if ($spouse_tb_list->DbMasterFilter <> "" && $spouse_tb->getCurrentMasterTable() == "personal_info") {
	if ($spouse_tb_list->MasterRecordExists) {
		if ($spouse_tb->getCurrentMasterTable() == $spouse_tb->TableVar) $gsMasterReturnUrl .= "?" . EW_TABLE_SHOW_MASTER . "=";
?>
<?php if ($spouse_tb_list->ExportOptions->Visible()) { ?>
...
if ($spouse_tb_list->DbMasterFilter <> "" && $spouse_tb->getCurrentMasterTable() == "premiumwill_tb") {
	if ($spouse_tb_list->MasterRecordExists) {
		if ($spouse_tb->getCurrentMasterTable() == $spouse_tb->TableVar) $gsMasterReturnUrl .= "?" . EW_TABLE_SHOW_MASTER . "=";
?>
<?php if ($spouse_tb_list->ExportOptions->Visible()) { ?>
...
if ($spouse_tb_list->DbMasterFilter <> "" && $spouse_tb->getCurrentMasterTable() == "privatetrust_tb") {
	if ($spouse_tb_list->MasterRecordExists) {
		if ($spouse_tb->getCurrentMasterTable() == $spouse_tb->TableVar) $gsMasterReturnUrl .= "?" . EW_TABLE_SHOW_MASTER . "=";
?>
<?php if ($spouse_tb_list->ExportOptions->Visible()) { ?>
...
<?php
	$bSelectLimit = EW_SELECT_LIMIT;
	if ($bSelectLimit) {
		$spouse_tb_list->TotalRecs = $spouse_tb->SelectRecordCount();
	} else {
		if ($spouse_tb_list->Recordset = $spouse_tb_list->LoadRecordset())
			$spouse_tb_list->TotalRecs = $spouse_tb_list->Recordset->RecordCount();
	}
	$spouse_tb_list->StartRec = 1;
	if ($spouse_tb_list->DisplayRecs <= 0 || ($spouse_tb->Export <> "" && $spouse_tb->ExportAll)) // Display all records
		$spouse_tb_list->DisplayRecs = $spouse_tb_list->TotalRecs;
	if (!($spouse_tb->Export <> "" && $spouse_tb->ExportAll))
		$spouse_tb_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$spouse_tb_list->Recordset = $spouse_tb_list->LoadRecordset($spouse_tb_list->StartRec-1, $spouse_tb_list->DisplayRecs);
$spouse_tb_list->RenderOtherOptions();
?>
<?php if ($Security->IsLoggedIn()) { ?>
<?php if ($spouse_tb->Export == "" && $spouse_tb->CurrentAction == "") { ?>
<form name="fspouse_tblistsrch" id="fspouse_tblistsrch" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>">
<table class="ewSearchTable"><tr><td>
<div class="accordion" id="fspouse_tblistsrch_SearchGroup">
	<div class="accordion-group">
		<div class="accordion-heading">
<a class="accordion-toggle" data-toggle="collapse" data-parent="#fspouse_tblistsrch_SearchGroup" href="#fspouse_tblistsrch_SearchBody"><?php echo $Language->Phrase("Search") ?></a>
		</div>
		<div id="fspouse_tblistsrch_SearchBody" class="accordion-body collapse in">
			<div class="accordion-inner">
<div id="fspouse_tblistsrch_SearchPanel">
...
<div id="xsr_1" class="ewRow">
	...
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="input-large" value="<?php echo ew_HtmlEncode($spouse_tb_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo $Language->Phrase("Search") ?>">
	<button class="btn btn-primary ewButton" name="btnsubmit" id="btnsubmit" type="submit"><?php echo $Language->Phrase("QuickSearchBtn") ?></button>
	</div>
	</div>
	<div class="btn-group ewButtonGroup">
	<a class="btn ewShowAll" href="<?php echo $spouse_tb_list->PageUrl() ?>cmd=reset"><?php echo $Language->Phrase("ShowAll") ?></a>
</div>
<div id="xsr_2" class="ewRow">
	<label class="inline radio ewRadio" style="white-space: nowrap;"><input type="radio" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="="<?php if ($spouse_tb_list->BasicSearch->getType() == "=") { ?> checked="checked"<?php } ?>><?php echo $Language->Phrase("ExactPhrase") ?></label>
	<label class="inline radio ewRadio" style="white-space: nowrap;"><input type="radio" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="AND"<?php if ($spouse_tb_list->BasicSearch->getType() == "AND") { ?> checked="checked"<?php } ?>><?php echo $Language->Phrase("AllWord") ?></label>
	<label class="inline radio ewRadio" style="white-space: nowrap;"><input type="radio" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="OR"<?php if ($spouse_tb_list->BasicSearch->getType() == "OR") { ?> checked="checked"<?php } ?>><?php echo $Language->Phrase("AnyWord") ?></label>
</div>
</div>
</div>
			</div>
...
<?php } ?>
<?php } ?>
<?php $spouse_tb_list->ShowPageHeader(); ?>
<?php
$spouse_tb_list->ShowMessage();
?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<form name="fspouse_tblist" id="fspouse_tblist" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="spouse_tb">
<div id="gmp_spouse_tb" class="ewGridMiddlePanel">
<?php if ($spouse_tb_list->TotalRecs > 0) { ?>
<table id="tbl_spouse_tblist" class="ewTable ewTableSeparate">
...
// Render list options
$spouse_tb_list->RenderListOptions();

// Render list options (header, left)
$spouse_tb_list->ListOptions->Render("header", "left");
?>
<?php if ($spouse_tb->id->Visible) { // id ?>
	<?php if ($spouse_tb->SortUrl($spouse_tb->id) == "") { ?>
		<td><div id="elh_spouse_tb_id" class="spouse_tb_id"><div class="ewTableHeaderCaption"><?php echo $spouse_tb->id->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $spouse_tb->SortUrl($spouse_tb->id) ?>',2);"><div id="elh_spouse_tb_id" class="spouse_tb_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $spouse_tb->id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($spouse_tb->id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($spouse_tb->id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($spouse_tb->title->Visible) { // title ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $spouse_tb->SortUrl($spouse_tb->title) ?>',2);"><div id="elh_spouse_tb_title" class="spouse_tb_title">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $spouse_tb->title->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($spouse_tb->title->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($spouse_tb->title->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($spouse_tb->fullname->Visible) { // fullname ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $spouse_tb->SortUrl($spouse_tb->fullname) ?>',2);"><div id="elh_spouse_tb_fullname" class="spouse_tb_fullname">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $spouse_tb->fullname->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($spouse_tb->fullname->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($spouse_tb->fullname->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($spouse_tb->_email->Visible) { // email ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $spouse_tb->SortUrl($spouse_tb->_email) ?>',2);"><div id="elh_spouse_tb__email" class="spouse_tb__email">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $spouse_tb->_email->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($spouse_tb->_email->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($spouse_tb->_email->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($spouse_tb->phoneno->Visible) { // phoneno ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $spouse_tb->SortUrl($spouse_tb->phoneno) ?>',2);"><div id="elh_spouse_tb_phoneno" class="spouse_tb_phoneno">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $spouse_tb->phoneno->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($spouse_tb->phoneno->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($spouse_tb->phoneno->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($spouse_tb->datecreated->Visible) { // datecreated ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $spouse_tb->SortUrl($spouse_tb->datecreated) ?>',2);"><div id="elh_spouse_tb_datecreated" class="spouse_tb_datecreated">
...
$spouse_tb_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($spouse_tb->ExportAll && $spouse_tb->Export <> "") {
	$spouse_tb_list->StopRec = $spouse_tb_list->TotalRecs;
} else {

	// Set the last record to display
	if ($spouse_tb_list->TotalRecs > $spouse_tb_list->StartRec + $spouse_tb_list->DisplayRecs - 1)
		$spouse_tb_list->StopRec = $spouse_tb_list->StartRec + $spouse_tb_list->DisplayRecs - 1;
	else
		$spouse_tb_list->StopRec = $spouse_tb_list->TotalRecs;
}
$spouse_tb_list->RecCnt = $spouse_tb_list->StartRec - 1;
if ($spouse_tb_list->Recordset && !$spouse_tb_list->Recordset->EOF) {
	$spouse_tb_list->Recordset->MoveFirst();
	if (!$bSelectLimit && $spouse_tb_list->StartRec > 1)
		$spouse_tb_list->Recordset->Move($spouse_tb_list->StartRec - 1);
} elseif (!$spouse_tb->AllowAddDeleteRow && $spouse_tb_list->StopRec == 0) {
	$spouse_tb_list->StopRec = $spouse_tb->GridAddRowCount;
}

// Initialize aggregate
$spouse_tb->RowType = EW_ROWTYPE_AGGREGATEINIT;
$spouse_tb->ResetAttrs();
$spouse_tb_list->RenderRow();
while ($spouse_tb_list->RecCnt < $spouse_tb_list->StopRec) {
	$spouse_tb_list->RecCnt++;
	if (intval($spouse_tb_list->RecCnt) >= intval($spouse_tb_list->StartRec)) {
		$spouse_tb_list->RowCnt++;

		// Set up key count
		$spouse_tb_list->KeyCount = $spouse_tb_list->RowIndex;
		...
		if ($spouse_tb->CurrentAction == "gridadd") {
			...
			$spouse_tb_list->LoadRowValues($spouse_tb_list->Recordset); // Load row values
		}
		$spouse_tb->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$spouse_tb->RowAttrs = array_merge($spouse_tb->RowAttrs, array('data-rowindex'=>$spouse_tb_list->RowCnt, 'id'=>'r' . $spouse_tb_list->RowCnt . '_spouse_tb', 'data-rowtype'=>$spouse_tb->RowType));
...
$spouse_tb_list->ListOptions->Render("body", "left", $spouse_tb_list->RowCnt);
?>
	<?php if ($spouse_tb->id->Visible) { // id ?>
		<td<?php echo $spouse_tb->id->CellAttributes() ?>>
<span<?php echo $spouse_tb->id->ViewAttributes() ?>>
<?php echo $spouse_tb->id->ListViewValue() ?></span>
<a id="<?php echo $spouse_tb_list->PageObjName . "_row_" . $spouse_tb_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($spouse_tb->title->Visible) { // title ?>
...
$spouse_tb_list->ListOptions->Render("body", "right", $spouse_tb_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($spouse_tb->CurrentAction <> "gridadd")
		$spouse_tb_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($spouse_tb->CurrentAction == "") { ?>
...
// Close recordset
if ($spouse_tb_list->Recordset)
	$spouse_tb_list->Recordset->Close();
?>
<?php if ($spouse_tb->Export == "") { ?>
<div class="ewGridLowerPanel">
<?php if ($spouse_tb->CurrentAction <> "gridadd" && $spouse_tb->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($spouse_tb_list->Pager)) $spouse_tb_list->Pager = new cNumericPager($spouse_tb_list->StartRec, $spouse_tb_list->DisplayRecs, $spouse_tb_list->TotalRecs, $spouse_tb_list->RecRange) ?>
<?php if ($spouse_tb_list->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($spouse_tb_list->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $spouse_tb_list->PageUrl() ?>start=<?php echo $spouse_tb_list->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($spouse_tb_list->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $spouse_tb_list->PageUrl() ?>start=<?php echo $spouse_tb_list->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($spouse_tb_list->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $spouse_tb_list->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($spouse_tb_list->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $spouse_tb_list->PageUrl() ?>start=<?php echo $spouse_tb_list->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($spouse_tb_list->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $spouse_tb_list->PageUrl() ?>start=<?php echo $spouse_tb_list->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
<td>
	<?php if ($spouse_tb_list->Pager->ButtonCount > 0) { ?>&nbsp;&nbsp;&nbsp;&nbsp;<?php } ?>
	<?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $spouse_tb_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $spouse_tb_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $spouse_tb_list->Pager->RecordCount ?>
</td>
</tr></tbody></table>
<?php } else { ?>
	<?php if ($spouse_tb_list->SearchWhere == "0=101") { ?>
...
<option value="25"<?php if ($spouse_tb_list->DisplayRecs == 25) { ?> selected="selected"<?php } ?>>25</option>
<option value="50"<?php if ($spouse_tb_list->DisplayRecs == 50) { ?> selected="selected"<?php } ?>>50</option>
<option value="100"<?php if ($spouse_tb_list->DisplayRecs == 100) { ?> selected="selected"<?php } ?>>100</option>
<option value="200"<?php if ($spouse_tb_list->DisplayRecs == 200) { ?> selected="selected"<?php } ?>>200</option>
<option value="500"<?php if ($spouse_tb_list->DisplayRecs == 500) { ?> selected="selected"<?php } ?>>500</option>
<option value="ALL"<?php if ($spouse_tb->getRecordsPerPage() == -1) { ?> selected="selected"<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</td>
<?php } ?>
</tr></table>
...
<?php
	foreach ($spouse_tb_list->OtherOptions as &$option)
