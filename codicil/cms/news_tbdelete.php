...
$news_tb_delete = NULL; // Initialize page object first

class cnews_tb_delete extends cnews_tb {
	...
	// Page object name
	var $PageObjName = 'news_tb_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("news_tblist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in news_tb class, news_tbinfo.php
...
// Create page object
if (!isset($news_tb_delete)) $news_tb_delete = new cnews_tb_delete();

// Page init
$news_tb_delete->Page_Init();

// Page main
$news_tb_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$news_tb_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var news_tb_delete = new ew_Page("news_tb_delete");
news_tb_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = news_tb_delete.PageID; // For backward compatibility

// Form object
var fnews_tbdelete = new ew_Form("fnews_tbdelete");

// Form_CustomValidate event
fnews_tbdelete.Form_CustomValidate = 
...
fnews_tbdelete.ValidateRequired = true;
<?php } else { ?>
fnews_tbdelete.ValidateRequired = false; 
<?php } ?>
...
if ($news_tb_delete->Recordset = $news_tb_delete->LoadRecordset())
	$news_tb_deleteTotalRecs = $news_tb_delete->Recordset->RecordCount(); // Get record count
if ($news_tb_deleteTotalRecs <= 0) { // No record found, exit
	if ($news_tb_delete->Recordset)
		$news_tb_delete->Recordset->Close();
	$news_tb_delete->Page_Terminate("news_tblist.php"); // Return to list
}
?>
<?php $Breadcrumb->Render(); ?>
<?php $news_tb_delete->ShowPageHeader(); ?>
<?php
$news_tb_delete->ShowMessage();
?>
<form name="fnews_tbdelete" id="fnews_tbdelete" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="news_tb">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($news_tb_delete->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_news_tbdelete" class="ewTable ewTableSeparate">
<?php echo $news_tb->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
<?php if ($news_tb->title->Visible) { // title ?>
		<td><span id="elh_news_tb_title" class="news_tb_title"><?php echo $news_tb->title->FldCaption() ?></span></td>
<?php } ?>
<?php if ($news_tb->postdate->Visible) { // postdate ?>
		<td><span id="elh_news_tb_postdate" class="news_tb_postdate"><?php echo $news_tb->postdate->FldCaption() ?></span></td>
<?php } ?>
<?php if ($news_tb->venue->Visible) { // venue ?>
		<td><span id="elh_news_tb_venue" class="news_tb_venue"><?php echo $news_tb->venue->FldCaption() ?></span></td>
<?php } ?>
<?php if ($news_tb->time->Visible) { // time ?>
		<td><span id="elh_news_tb_time" class="news_tb_time"><?php echo $news_tb->time->FldCaption() ?></span></td>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$news_tb_delete->RecCnt = 0;
$i = 0;
while (!$news_tb_delete->Recordset->EOF) {
	$news_tb_delete->RecCnt++;
	$news_tb_delete->RowCnt++;

	// Set row properties
	$news_tb->ResetAttrs();
	$news_tb->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$news_tb_delete->LoadRowValues($news_tb_delete->Recordset);

	// Render row
	$news_tb_delete->RenderRow();
?>
	<tr<?php echo $news_tb->RowAttributes() ?>>
<?php if ($news_tb->title->Visible) { // title ?>
		<td<?php echo $news_tb->title->CellAttributes() ?>>
<span id="el<?php echo $news_tb_delete->RowCnt ?>_news_tb_title" class="control-group news_tb_title">
...
<span id="el<?php echo $news_tb_delete->RowCnt ?>_news_tb_postdate" class="control-group news_tb_postdate">
...
<span id="el<?php echo $news_tb_delete->RowCnt ?>_news_tb_venue" class="control-group news_tb_venue">
...
<span id="el<?php echo $news_tb_delete->RowCnt ?>_news_tb_time" class="control-group news_tb_time">
...
<?php
	$news_tb_delete->Recordset->MoveNext();
}
$news_tb_delete->Recordset->Close();
?>
</tbody>
