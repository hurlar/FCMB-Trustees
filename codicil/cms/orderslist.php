...
class corders_list extends corders {
	...
	function AdvancedSearchWhere() {
		...
		$this->BuildSearchSql($sWhere, $this->customerid, FALSE); // customerid
		$this->BuildSearchSql($sWhere, $this->ship, FALSE); // ship
		...
		if ($this->Command == "search") {
			...
			$this->ship->AdvancedSearch->Save(); // ship
	...
	function BasicSearchSQL($Keyword) {
		...
		$sWhere = "";
		$this->BuildBasicSearchSQL($sWhere, $this->ship, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $this->total, $Keyword);
		return $sWhere;
	}
	...
	function CheckSearchParms() {
		...
		if ($this->customerid->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->ship->AdvancedSearch->IssetSession())
			return TRUE;
	...
	function ResetAdvancedSearchParms() {
		...
		$this->ship->AdvancedSearch->UnsetSession();
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->ship, $bCtrl); // ship
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewAction ewMultiDelete\" href=\"\" onclick=\"ew_SubmitSelected(document.forderslist, '" . $this->MultiDeleteUrl . "', ewLanguage.Phrase('DeleteMultiConfirmMsg'));return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = ($Security->IsLoggedIn());
	...
	function LoadSearchValues() {
		...
		// customerid
		$this->customerid->AdvancedSearch->SearchValue = ew_StripSlashes(@$_GET["x_customerid"]);
		if ($this->customerid->AdvancedSearch->SearchValue <> "") $this->Command = "search";
		$this->customerid->AdvancedSearch->SearchOperator = @$_GET["z_customerid"];

		// ship
		$this->ship->AdvancedSearch->SearchValue = ew_StripSlashes(@$_GET["x_ship"]);
		if ($this->ship->AdvancedSearch->SearchValue <> "") $this->Command = "search";
		$this->ship->AdvancedSearch->SearchOperator = @$_GET["z_ship"];
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// ship
			$this->ship->EditCustomAttributes = "";
			$this->ship->EditValue = ew_HtmlEncode($this->ship->AdvancedSearch->SearchValue);
			$this->ship->PlaceHolder = ew_HtmlEncode(ew_RemoveHtml($this->ship->FldCaption()));

			// total
			$this->total->EditCustomAttributes = "";
			$this->total->EditValue = ew_HtmlEncode($this->total->AdvancedSearch->SearchValue);
			$this->total->PlaceHolder = ew_HtmlEncode(ew_RemoveHtml($this->total->FldCaption()));
			...
			$this->date->EditValue = ew_HtmlEncode(ew_UnFormatDateTime($this->date->AdvancedSearch->SearchValue, 0));
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_orders\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_orders',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.forderslist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
	...
	function ExportQueryString() {
		...
		$this->AddSearchQueryString($sQry, $this->customerid); // customerid
		$this->AddSearchQueryString($sQry, $this->ship); // ship
		$this->AddSearchQueryString($sQry, $this->total); // total
		$this->AddSearchQueryString($sQry, $this->status); // status
		$this->AddSearchQueryString($sQry, $this->date); // date
...
forderslist.ValidateRequired = true;
<?php } else { ?>
forderslist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
forderslist.Lists["x_customerid"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_name","x_lname","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
// Use JavaScript validation or not
<?php if (EW_CLIENT_VALIDATE) { ?>
forderslistsrch.ValidateRequired = true; // Use JavaScript validation
<?php } else { ?>
forderslistsrch.ValidateRequired = false; // No JavaScript validation
<?php } ?>
...
<?php } ?>
<?php if ($orders_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $orders_list->ExportOptions->Render("body") ?></div>
<?php } ?>
<?php
	$bSelectLimit = EW_SELECT_LIMIT;
	if ($bSelectLimit) {
		$orders_list->TotalRecs = $orders->SelectRecordCount();
	} else {
		if ($orders_list->Recordset = $orders_list->LoadRecordset())
...
<form name="forderslistsrch" id="forderslistsrch" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>">
<table class="ewSearchTable"><tr><td>
<div class="accordion" id="forderslistsrch_SearchGroup">
	<div class="accordion-group">
		<div class="accordion-heading">
<a class="accordion-toggle" data-toggle="collapse" data-parent="#forderslistsrch_SearchGroup" href="#forderslistsrch_SearchBody"><?php echo $Language->Phrase("Search") ?></a>
		</div>
		<div id="forderslistsrch_SearchBody" class="accordion-body collapse in">
			<div class="accordion-inner">
<div id="forderslistsrch_SearchPanel">
...
<?php if ($orders->status->Visible) { // status ?>
	<span id="xsc_status" class="ewCell">
		<span class="ewSearchCaption"><?php echo $orders->status->FldCaption() ?></span>
		<span class="ewSearchOperator"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_status" id="z_status" value="="></span>
		<span class="control-group ewSearchField">
<div id="tp_x_status" class="<?php echo EW_ITEM_TEMPLATE_CLASSNAME ?>"><input type="radio" name="x_status" id="x_status" value="{value}"<?php echo $orders->status->EditAttributes() ?>></div>
<div id="dsl_x_status" data-repeatcolumn="5" class="ewItemList">
<?php
$arwrk = $orders->status->EditValue;
if (is_array($arwrk)) {
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($orders->status->AdvancedSearch->SearchValue) == strval($arwrk[$rowcntwrk][0])) ? " checked=\"checked\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;

		// Note: No spacing within the LABEL tag
?>
<?php echo ew_RepeatColumnTable($rowswrk, $rowcntwrk, 5, 1) ?>
<label class="radio"><input type="radio" data-field="x_status" name="x_status" id="x_status_<?php echo $rowcntwrk ?>" value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?><?php echo $orders->status->EditAttributes() ?>><?php echo $arwrk[$rowcntwrk][1] ?></label>
...
<div id="xsr_2" class="ewRow">
	...
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="input-large" value="<?php echo ew_HtmlEncode($orders_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo $Language->Phrase("Search") ?>">
	<button class="btn btn-primary ewButton" name="btnsubmit" id="btnsubmit" type="submit"><?php echo $Language->Phrase("QuickSearchBtn") ?></button>
	</div>
	</div>
	<div class="btn-group ewButtonGroup">
	<a class="btn ewShowAll" href="<?php echo $orders_list->PageUrl() ?>cmd=reset"><?php echo $Language->Phrase("ShowAll") ?></a>
</div>
<div id="xsr_3" class="ewRow">
	<label class="inline radio ewRadio" style="white-space: nowrap;"><input type="radio" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="="<?php if ($orders_list->BasicSearch->getType() == "=") { ?> checked="checked"<?php } ?>><?php echo $Language->Phrase("ExactPhrase") ?></label>
	<label class="inline radio ewRadio" style="white-space: nowrap;"><input type="radio" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="AND"<?php if ($orders_list->BasicSearch->getType() == "AND") { ?> checked="checked"<?php } ?>><?php echo $Language->Phrase("AllWord") ?></label>
	<label class="inline radio ewRadio" style="white-space: nowrap;"><input type="radio" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="OR"<?php if ($orders_list->BasicSearch->getType() == "OR") { ?> checked="checked"<?php } ?>><?php echo $Language->Phrase("AnyWord") ?></label>
</div>
</div>
</div>
			</div>
...
<form name="forderslist" id="forderslist" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="orders">
<div id="gmp_orders" class="ewGridMiddlePanel">
<?php if ($orders_list->TotalRecs > 0) { ?>
<table id="tbl_orderslist" class="ewTable ewTableSeparate">
...
<?php if ($orders->customerid->Visible) { // customerid ?>
	<?php if ($orders->SortUrl($orders->customerid) == "") { ?>
		<td><div id="elh_orders_customerid" class="orders_customerid"><div class="ewTableHeaderCaption"><?php echo $orders->customerid->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $orders->SortUrl($orders->customerid) ?>',2);"><div id="elh_orders_customerid" class="orders_customerid">
...
<?php if ($orders->ship->Visible) { // ship ?>
	<?php if ($orders->SortUrl($orders->ship) == "") { ?>
		<td><div id="elh_orders_ship" class="orders_ship"><div class="ewTableHeaderCaption"><?php echo $orders->ship->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $orders->SortUrl($orders->ship) ?>',2);"><div id="elh_orders_ship" class="orders_ship">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $orders->ship->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($orders->ship->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($orders->ship->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($orders->total->Visible) { // total ?>
	<?php if ($orders->SortUrl($orders->total) == "") { ?>
		<td><div id="elh_orders_total" class="orders_total"><div class="ewTableHeaderCaption"><?php echo $orders->total->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $orders->SortUrl($orders->total) ?>',2);"><div id="elh_orders_total" class="orders_total">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $orders->total->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($orders->total->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($orders->total->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($orders->status->Visible) { // status ?>
	<?php if ($orders->SortUrl($orders->status) == "") { ?>
		<td><div id="elh_orders_status" class="orders_status"><div class="ewTableHeaderCaption"><?php echo $orders->status->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $orders->SortUrl($orders->status) ?>',2);"><div id="elh_orders_status" class="orders_status">
...
<?php if ($orders->date->Visible) { // date ?>
	<?php if ($orders->SortUrl($orders->date) == "") { ?>
		<td><div id="elh_orders_date" class="orders_date"><div class="ewTableHeaderCaption"><?php echo $orders->date->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $orders->SortUrl($orders->date) ?>',2);"><div id="elh_orders_date" class="orders_date">
...
<a id="<?php echo $orders_list->PageObjName . "_row_" . $orders_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($orders->ship->Visible) { // ship ?>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($orders_list->Pager)) $orders_list->Pager = new cNumericPager($orders_list->StartRec, $orders_list->DisplayRecs, $orders_list->TotalRecs, $orders_list->RecRange) ?>
<?php if ($orders_list->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($orders_list->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $orders_list->PageUrl() ?>start=<?php echo $orders_list->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($orders_list->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $orders_list->PageUrl() ?>start=<?php echo $orders_list->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($orders_list->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $orders_list->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($orders_list->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $orders_list->PageUrl() ?>start=<?php echo $orders_list->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($orders_list->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $orders_list->PageUrl() ?>start=<?php echo $orders_list->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
<td>
	<?php if ($orders_list->Pager->ButtonCount > 0) { ?>&nbsp;&nbsp;&nbsp;&nbsp;<?php } ?>
	<?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $orders_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $orders_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $orders_list->Pager->RecordCount ?>
</td>
</tr></tbody></table>
<?php } else { ?>
	<?php if ($orders_list->SearchWhere == "0=101") { ?>
...
<option value="25"<?php if ($orders_list->DisplayRecs == 25) { ?> selected="selected"<?php } ?>>25</option>
<option value="50"<?php if ($orders_list->DisplayRecs == 50) { ?> selected="selected"<?php } ?>>50</option>
<option value="100"<?php if ($orders_list->DisplayRecs == 100) { ?> selected="selected"<?php } ?>>100</option>
<option value="200"<?php if ($orders_list->DisplayRecs == 200) { ?> selected="selected"<?php } ?>>200</option>
<option value="500"<?php if ($orders_list->DisplayRecs == 500) { ?> selected="selected"<?php } ?>>500</option>
