...
$products_img4_blobview = NULL; // Initialize page object first

class cproducts_img4_blobview extends cproducts {
	...
	// Page object name
	var $PageObjName = 'products_img4_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('products', 'x_img4');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('img4');
				...
				if (trim(strval($this->Recordset->fields('img4'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('img4') . "\"");
				}
...
// Create page object
if (!isset($products_img4_blobview)) $products_img4_blobview = new cproducts_img4_blobview();

// Page init
$products_img4_blobview->Page_Init();

// Page main
$products_img4_blobview->Page_Main();
?>
<?php
$products_img4_blobview->Page_Terminate();
?>
