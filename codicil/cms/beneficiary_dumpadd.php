...
class cbeneficiary_dump_add extends cbeneficiary_dump {
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			...
			if (in_array("alt_beneficiary", $DetailTblVar) && $GLOBALS["alt_beneficiary"]->DetailAdd) {
				$GLOBALS["alt_beneficiary"]->childid->setSessionValue($this->childid->CurrentValue); // Set master key
				if (!isset($GLOBALS["alt_beneficiary_grid"])) $GLOBALS["alt_beneficiary_grid"] = new calt_beneficiary_grid(); // Get detail page object
				$AddRow = $GLOBALS["alt_beneficiary_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["alt_beneficiary"]->childid->setSessionValue(""); // Clear master key if insert failed
			}
		}
...
<?php } ?>
<?php
	if (in_array("alt_beneficiary", explode(",", $beneficiary_dump->getCurrentDetailTable())) && $alt_beneficiary->DetailAdd) {
?>
