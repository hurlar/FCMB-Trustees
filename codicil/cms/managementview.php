...
class cmanagement_view extends cmanagement {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("managementlist.php"); // Return to list page
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_management\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_management',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fmanagementview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Page object
var management_view = new ew_Page("management_view");
management_view.PageID = "view"; // Page ID
var EW_PAGE_ID = management_view.PageID; // For backward compatibility

// Form object
var fmanagementview = new ew_Form("fmanagementview");
...
fmanagementview.ValidateRequired = true;
<?php } else { ?>
fmanagementview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmanagementview.Lists["x_category"] = {"LinkField":"x_category","Ajax":null,"AutoFill":false,"DisplayFields":["x_category","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php if (!$management_view->ExportOptions->UseDropDownButton) { ?>
</div>
...
?>
<form name="fmanagementview" id="fmanagementview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="management">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_managementview" class="table table-bordered table-striped">
<?php if ($management->id->Visible) { // id ?>
	<tr id="r_id">
		<td><span id="elh_management_id"><?php echo $management->id->FldCaption() ?></span></td>
		<td<?php echo $management->id->CellAttributes() ?>>
<span id="el_management_id" class="control-group">
...
<?php if ($management->name->Visible) { // name ?>
	<tr id="r_name">
		<td><span id="elh_management_name"><?php echo $management->name->FldCaption() ?></span></td>
...
<?php echo $management->name->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $management->position->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<span<?php echo $management->image->ViewAttributes() ?>>
<?php if ($management->image->LinkAttributes() <> "") { ?>
<?php if (!empty($management->image->Upload->DbValue)) { ?>
<a<?php echo $management->image->LinkAttributes() ?>><?php echo $management->image->ViewValue ?></a>
<?php } elseif (!in_array($management->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!empty($management->image->Upload->DbValue)) { ?>
<?php echo $management->image->ViewValue ?>
<?php } elseif (!in_array($management->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } ?>
...
<span<?php echo $management->descrp->ViewAttributes() ?>>
<?php echo $management->descrp->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($management->slug->Visible) { // slug ?>
	<tr id="r_slug">
		<td><span id="elh_management_slug"><?php echo $management->slug->FldCaption() ?></span></td>
...
<?php echo $management->slug->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($management->category->Visible) { // category ?>
	<tr id="r_category">
		<td><span id="elh_management_category"><?php echo $management->category->FldCaption() ?></span></td>
...
<?php echo $management->category->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $management->rate_ord->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($management_view->Pager)) $management_view->Pager = new cNumericPager($management_view->StartRec, $management_view->DisplayRecs, $management_view->TotalRecs, $management_view->RecRange) ?>
<?php if ($management_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($management_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $management_view->PageUrl() ?>start=<?php echo $management_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($management_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $management_view->PageUrl() ?>start=<?php echo $management_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($management_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $management_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($management_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $management_view->PageUrl() ?>start=<?php echo $management_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($management_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $management_view->PageUrl() ?>start=<?php echo $management_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
