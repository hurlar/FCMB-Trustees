...
$spouse_tb_view = NULL; // Initialize page object first

class cspouse_tb_view extends cspouse_tb {
	...
	// Page object name
	var $PageObjName = 'spouse_tb_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "spouse_tblist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "spouse_tblist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		$body = "<a class=\"ewAction ewDetailList\" href=\"" . ew_HtmlEncode("children_detailslist.php?" . EW_TABLE_SHOW_MASTER . "=spouse_tb&uid=" . strval($this->uid->CurrentValue) . "") . "\">" . $body . "</a>";
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_spouse_tb\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_spouse_tb',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fspouse_tbview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($spouse_tb_view)) $spouse_tb_view = new cspouse_tb_view();

// Page init
$spouse_tb_view->Page_Init();

// Page main
$spouse_tb_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$spouse_tb_view->Page_Render();
?>
...
// Page object
var spouse_tb_view = new ew_Page("spouse_tb_view");
spouse_tb_view.PageID = "view"; // Page ID
var EW_PAGE_ID = spouse_tb_view.PageID; // For backward compatibility

// Form object
var fspouse_tbview = new ew_Form("fspouse_tbview");

// Form_CustomValidate event
fspouse_tbview.Form_CustomValidate = 
...
fspouse_tbview.ValidateRequired = true;
<?php } else { ?>
fspouse_tbview.ValidateRequired = false; 
<?php } ?>
...
<?php $spouse_tb_view->ExportOptions->Render("body") ?>
<?php if (!$spouse_tb_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($spouse_tb_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $spouse_tb_view->ShowPageHeader(); ?>
<?php
$spouse_tb_view->ShowMessage();
?>
<form name="fspouse_tbview" id="fspouse_tbview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="spouse_tb">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_spouse_tbview" class="table table-bordered table-striped">
...
<?php echo $spouse_tb->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($spouse_tb->uid->Visible) { // uid ?>
	<tr id="r_uid">
		<td><span id="elh_spouse_tb_uid"><?php echo $spouse_tb->uid->FldCaption() ?></span></td>
		<td<?php echo $spouse_tb->uid->CellAttributes() ?>>
<span id="el_spouse_tb_uid" class="control-group">
...
<span<?php echo $spouse_tb->dob->ViewAttributes() ?>>
<?php echo $spouse_tb->dob->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<span<?php echo $spouse_tb->altphoneno->ViewAttributes() ?>>
<?php echo $spouse_tb->altphoneno->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<span<?php echo $spouse_tb->addr->ViewAttributes() ?>>
<?php echo $spouse_tb->addr->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<span<?php echo $spouse_tb->city->ViewAttributes() ?>>
<?php echo $spouse_tb->city->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<span<?php echo $spouse_tb->state->ViewAttributes() ?>>
<?php echo $spouse_tb->state->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($spouse_tb->marriagetype->Visible) { // marriagetype ?>
	<tr id="r_marriagetype">
		<td><span id="elh_spouse_tb_marriagetype"><?php echo $spouse_tb->marriagetype->FldCaption() ?></span></td>
		<td<?php echo $spouse_tb->marriagetype->CellAttributes() ?>>
<span id="el_spouse_tb_marriagetype" class="control-group">
<span<?php echo $spouse_tb->marriagetype->ViewAttributes() ?>>
<?php echo $spouse_tb->marriagetype->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($spouse_tb->marriageyear->Visible) { // marriageyear ?>
	<tr id="r_marriageyear">
		<td><span id="elh_spouse_tb_marriageyear"><?php echo $spouse_tb->marriageyear->FldCaption() ?></span></td>
		<td<?php echo $spouse_tb->marriageyear->CellAttributes() ?>>
<span id="el_spouse_tb_marriageyear" class="control-group">
<span<?php echo $spouse_tb->marriageyear->ViewAttributes() ?>>
<?php echo $spouse_tb->marriageyear->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($spouse_tb->marriagecert->Visible) { // marriagecert ?>
	<tr id="r_marriagecert">
		<td><span id="elh_spouse_tb_marriagecert"><?php echo $spouse_tb->marriagecert->FldCaption() ?></span></td>
		<td<?php echo $spouse_tb->marriagecert->CellAttributes() ?>>
<span id="el_spouse_tb_marriagecert" class="control-group">
<span<?php echo $spouse_tb->marriagecert->ViewAttributes() ?>>
<?php echo $spouse_tb->marriagecert->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<span<?php echo $spouse_tb->citym->ViewAttributes() ?>>
<?php echo $spouse_tb->citym->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<span<?php echo $spouse_tb->countrym->ViewAttributes() ?>>
<?php echo $spouse_tb->countrym->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($spouse_tb_view->Pager)) $spouse_tb_view->Pager = new cNumericPager($spouse_tb_view->StartRec, $spouse_tb_view->DisplayRecs, $spouse_tb_view->TotalRecs, $spouse_tb_view->RecRange) ?>
<?php if ($spouse_tb_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($spouse_tb_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $spouse_tb_view->PageUrl() ?>start=<?php echo $spouse_tb_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($spouse_tb_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $spouse_tb_view->PageUrl() ?>start=<?php echo $spouse_tb_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($spouse_tb_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $spouse_tb_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($spouse_tb_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $spouse_tb_view->PageUrl() ?>start=<?php echo $spouse_tb_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($spouse_tb_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $spouse_tb_view->PageUrl() ?>start=<?php echo $spouse_tb_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<?php } ?>
<?php
	if (in_array("children_details", explode(",", $spouse_tb->getCurrentDetailTable())) && $children_details->DetailView) {
?>
