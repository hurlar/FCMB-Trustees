...
$content_uploads_blobview = NULL; // Initialize page object first

class ccontent_uploads_blobview extends ccontent {
	...
	// Page object name
	var $PageObjName = 'content_uploads_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('content', 'x_uploads');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('uploads');
				...
				if (trim(strval($this->Recordset->fields('uploads'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('uploads') . "\"");
				}
...
// Create page object
if (!isset($content_uploads_blobview)) $content_uploads_blobview = new ccontent_uploads_blobview();

// Page init
$content_uploads_blobview->Page_Init();

// Page main
$content_uploads_blobview->Page_Main();
?>
<?php
$content_uploads_blobview->Page_Terminate();
?>
