...
$simplewill_tb_edit = NULL; // Initialize page object first

class csimplewill_tb_edit extends csimplewill_tb {
	...
	// Page object name
	var $PageObjName = 'simplewill_tb_edit';
	...
	function ValidateForm() {
		...
		if (in_array("simplewill_assets_tb", $DetailTblVar) && $GLOBALS["simplewill_assets_tb"]->DetailEdit) {
		...
		if ($rs->EOF) {
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					...
					if (in_array("simplewill_assets_tb", $DetailTblVar) && $GLOBALS["simplewill_assets_tb"]->DetailEdit) {
						if (!isset($GLOBALS["simplewill_assets_tb_grid"])) $GLOBALS["simplewill_assets_tb_grid"] = new csimplewill_assets_tb_grid(); // Get detail page object
						$EditRow = $GLOBALS["simplewill_assets_tb_grid"]->GridUpdate();
					}
					if (in_array("simplewill_overall_asset", $DetailTblVar) && $GLOBALS["simplewill_overall_asset"]->DetailEdit) {
						if (!isset($GLOBALS["simplewill_overall_asset_grid"])) $GLOBALS["simplewill_overall_asset_grid"] = new csimplewill_overall_asset_grid(); // Get detail page object
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("simplewill_assets_tb", $DetailTblVar)) {
				...
				if ($GLOBALS["simplewill_assets_tb_grid"]->DetailEdit) {
					$GLOBALS["simplewill_assets_tb_grid"]->CurrentMode = "edit";
					$GLOBALS["simplewill_assets_tb_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($simplewill_tb_edit)) $simplewill_tb_edit = new csimplewill_tb_edit();

// Page init
$simplewill_tb_edit->Page_Init();

// Page main
$simplewill_tb_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$simplewill_tb_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var simplewill_tb_edit = new ew_Page("simplewill_tb_edit");
simplewill_tb_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = simplewill_tb_edit.PageID; // For backward compatibility

// Form object
var fsimplewill_tbedit = new ew_Form("fsimplewill_tbedit");

// Validate form
fsimplewill_tbedit.Validate = function() {
...
fsimplewill_tbedit.Form_CustomValidate = 
...
fsimplewill_tbedit.ValidateRequired = true;
<?php } else { ?>
fsimplewill_tbedit.ValidateRequired = false; 
<?php } ?>
...
<?php $simplewill_tb_edit->ShowPageHeader(); ?>
<?php
$simplewill_tb_edit->ShowMessage();
?>
<form name="fsimplewill_tbedit" id="fsimplewill_tbedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="simplewill_tb">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_simplewill_tbedit" class="table table-bordered table-striped">
...
<?php echo $simplewill_tb->id->EditValue ?></span>
</span>
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($simplewill_tb->id->CurrentValue) ?>">
<?php echo $simplewill_tb->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($simplewill_tb_edit->Pager)) $simplewill_tb_edit->Pager = new cNumericPager($simplewill_tb_edit->StartRec, $simplewill_tb_edit->DisplayRecs, $simplewill_tb_edit->TotalRecs, $simplewill_tb_edit->RecRange) ?>
<?php if ($simplewill_tb_edit->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($simplewill_tb_edit->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $simplewill_tb_edit->PageUrl() ?>start=<?php echo $simplewill_tb_edit->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($simplewill_tb_edit->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $simplewill_tb_edit->PageUrl() ?>start=<?php echo $simplewill_tb_edit->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($simplewill_tb_edit->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $simplewill_tb_edit->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($simplewill_tb_edit->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $simplewill_tb_edit->PageUrl() ?>start=<?php echo $simplewill_tb_edit->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($simplewill_tb_edit->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $simplewill_tb_edit->PageUrl() ?>start=<?php echo $simplewill_tb_edit->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<?php
	if (in_array("simplewill_assets_tb", explode(",", $simplewill_tb->getCurrentDetailTable())) && $simplewill_assets_tb->DetailEdit) {
?>
<?php include_once "simplewill_assets_tbgrid.php" ?>
<?php } ?>
<?php
	if (in_array("simplewill_overall_asset", explode(",", $simplewill_tb->getCurrentDetailTable())) && $simplewill_overall_asset->DetailEdit) {
?>
...
<script type="text/javascript">
fsimplewill_tbedit.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$simplewill_tb_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$simplewill_tb_edit->Page_Terminate();
?>
