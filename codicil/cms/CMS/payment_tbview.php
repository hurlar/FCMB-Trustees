...
$payment_tb_view = NULL; // Initialize page object first

class cpayment_tb_view extends cpayment_tb {
	...
	// Page object name
	var $PageObjName = 'payment_tb_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "payment_tblist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "payment_tblist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_payment_tb\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_payment_tb',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fpayment_tbview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($payment_tb_view)) $payment_tb_view = new cpayment_tb_view();

// Page init
$payment_tb_view->Page_Init();

// Page main
$payment_tb_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$payment_tb_view->Page_Render();
?>
...
// Page object
var payment_tb_view = new ew_Page("payment_tb_view");
payment_tb_view.PageID = "view"; // Page ID
var EW_PAGE_ID = payment_tb_view.PageID; // For backward compatibility

// Form object
var fpayment_tbview = new ew_Form("fpayment_tbview");

// Form_CustomValidate event
fpayment_tbview.Form_CustomValidate = 
...
fpayment_tbview.ValidateRequired = true;
<?php } else { ?>
fpayment_tbview.ValidateRequired = false; 
<?php } ?>
...
<?php $payment_tb_view->ExportOptions->Render("body") ?>
<?php if (!$payment_tb_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($payment_tb_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $payment_tb_view->ShowPageHeader(); ?>
<?php
$payment_tb_view->ShowMessage();
?>
<form name="fpayment_tbview" id="fpayment_tbview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="payment_tb">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_payment_tbview" class="table table-bordered table-striped">
...
<?php echo $payment_tb->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<span<?php echo $payment_tb->uid->ViewAttributes() ?>>
<?php echo $payment_tb->uid->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $payment_tb->name->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $payment_tb->transactionid->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $payment_tb->willtype->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $payment_tb->amount->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $payment_tb->datepaid->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<table class="ewPager">
<tr><td>
<?php if (!isset($payment_tb_view->Pager)) $payment_tb_view->Pager = new cNumericPager($payment_tb_view->StartRec, $payment_tb_view->DisplayRecs, $payment_tb_view->TotalRecs, $payment_tb_view->RecRange) ?>
<?php if ($payment_tb_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($payment_tb_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $payment_tb_view->PageUrl() ?>start=<?php echo $payment_tb_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($payment_tb_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $payment_tb_view->PageUrl() ?>start=<?php echo $payment_tb_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($payment_tb_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $payment_tb_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($payment_tb_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $payment_tb_view->PageUrl() ?>start=<?php echo $payment_tb_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($payment_tb_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $payment_tb_view->PageUrl() ?>start=<?php echo $payment_tb_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<script type="text/javascript">
fpayment_tbview.Init();
</script>
<?php
$payment_tb_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($payment_tb->Export == "") { ?>
...
<?php
$payment_tb_view->Page_Terminate();
?>
