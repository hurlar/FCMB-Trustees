...
<?php include_once "nextofkingridcls.php" ?>
...
class cpersonal_info_list extends cpersonal_info {
	...
	function SetupListOptions() {
		...
		// "detail_nextofkin"
		$item = &$this->ListOptions->Add("detail_nextofkin");
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = $Security->IsLoggedIn() && !$this->ShowMultipleDetails;
		$item->OnLeft = FALSE;
		$item->ShowInButtonGroup = FALSE;
		if (!isset($GLOBALS["nextofkin_grid"])) $GLOBALS["nextofkin_grid"] = new cnextofkin_grid;
	...
	function RenderListOptions() {
		...
		// "detail_nextofkin"
		$oListOpt = &$this->ListOptions->Items["detail_nextofkin"];
		if ($Security->IsLoggedIn()) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("nextofkin", "TblCaption");
			$body = "<a class=\"btn btn-small ewRowLink ewDetailList\" data-action=\"list\" href=\"" . ew_HtmlEncode("nextofkinlist.php?" . EW_TABLE_SHOW_MASTER . "=personal_info&uid=" . strval($this->uid->CurrentValue) . "") . "\">" . $body . "</a>";
			$links = "";
			if ($GLOBALS["nextofkin_grid"]->DetailView && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
				$links .= "<li><a class=\"ewRowLink ewDetailView\" data-action=\"view\" href=\"" . ew_HtmlEncode($this->GetViewUrl(EW_TABLE_SHOW_DETAIL . "=nextofkin")) . "\">" . $Language->Phrase("MasterDetailViewLink") . "</a></li>";
				if ($DetailViewTblVar <> "") $DetailViewTblVar .= ",";
				$DetailViewTblVar .= "nextofkin";
			}
			if ($links <> "") {
				$body .= "<button class=\"btn btn-small dropdown-toggle\" data-toggle=\"dropdown\"><b class=\"caret\"></b></button>";
				$body .= "<ul class=\"dropdown-menu\">". $links . "</ul>";
	...
	function RenderListOptionsExt() {
		...
		// Column "detail_nextofkin"
		$link = "";
		$option = &$this->ListOptions->Items["detail_nextofkin"];
		$url = "nextofkinpreview.php?f=" . ew_Encrypt($sSqlWrk);
		$btngrp = "<div data-table=\"nextofkin\" data-url=\"" . $url . "\" class=\"btn-group\">";
		if ($Security->IsLoggedIn()) {			
			$label = $Language->TablePhrase("nextofkin", "TblCaption");
			$link = "<li><a href=\"#\" data-toggle=\"tab\" data-table=\"nextofkin\" data-url=\"" . $url . "\">" . $label . "</a></li>";			
			$links .= $link;
			$detaillnk = ew_JsEncode3("nextofkinlist.php?" . EW_TABLE_SHOW_MASTER . "=personal_info&uid=" . urlencode(strval($this->uid->CurrentValue)) . "");
			$btngrp .= "<button type=\"button\" class=\"btn btn-small\" onclick=\"window.location='" . $detaillnk . "'\">" . $Language->TablePhrase("nextofkin", "TblCaption") . "</button>";
		}
		if ($GLOBALS["nextofkin_grid"]->DetailView && $Security->IsLoggedIn() && $Security->IsLoggedIn())
			$btngrp .= "<button type=\"button\" class=\"btn btn-small\" onclick=\"window.location='" . $this->GetViewUrl(EW_TABLE_SHOW_DETAIL . "=nextofkin") . "'\">" . $Language->Phrase("MasterDetailViewLink") . "</button>";
		$btngrp .= "</div>";
		if ($link <> "") {
			$btngrps .= $btngrp;
			$option->Body .= "<div class=\"hide ewPreview\">" . $link . $btngrp . "</div>";
