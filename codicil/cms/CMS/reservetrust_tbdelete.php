...
$reservetrust_tb_delete = NULL; // Initialize page object first

class creservetrust_tb_delete extends creservetrust_tb {
	...
	// Page object name
	var $PageObjName = 'reservetrust_tb_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("reservetrust_tblist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in reservetrust_tb class, reservetrust_tbinfo.php
...
// Create page object
if (!isset($reservetrust_tb_delete)) $reservetrust_tb_delete = new creservetrust_tb_delete();

// Page init
$reservetrust_tb_delete->Page_Init();

// Page main
$reservetrust_tb_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$reservetrust_tb_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var reservetrust_tb_delete = new ew_Page("reservetrust_tb_delete");
reservetrust_tb_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = reservetrust_tb_delete.PageID; // For backward compatibility

// Form object
var freservetrust_tbdelete = new ew_Form("freservetrust_tbdelete");

// Form_CustomValidate event
freservetrust_tbdelete.Form_CustomValidate = 
...
freservetrust_tbdelete.ValidateRequired = true;
<?php } else { ?>
freservetrust_tbdelete.ValidateRequired = false; 
<?php } ?>
...
if ($reservetrust_tb_delete->Recordset = $reservetrust_tb_delete->LoadRecordset())
	$reservetrust_tb_deleteTotalRecs = $reservetrust_tb_delete->Recordset->RecordCount(); // Get record count
if ($reservetrust_tb_deleteTotalRecs <= 0) { // No record found, exit
	if ($reservetrust_tb_delete->Recordset)
		$reservetrust_tb_delete->Recordset->Close();
	$reservetrust_tb_delete->Page_Terminate("reservetrust_tblist.php"); // Return to list
}
?>
<?php $Breadcrumb->Render(); ?>
<?php $reservetrust_tb_delete->ShowPageHeader(); ?>
<?php
$reservetrust_tb_delete->ShowMessage();
?>
<form name="freservetrust_tbdelete" id="freservetrust_tbdelete" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="reservetrust_tb">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($reservetrust_tb_delete->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_reservetrust_tbdelete" class="ewTable ewTableSeparate">
<?php echo $reservetrust_tb->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
<?php if ($reservetrust_tb->fullname->Visible) { // fullname ?>
		<td><span id="elh_reservetrust_tb_fullname" class="reservetrust_tb_fullname"><?php echo $reservetrust_tb->fullname->FldCaption() ?></span></td>
<?php } ?>
<?php if ($reservetrust_tb->_email->Visible) { // email ?>
		<td><span id="elh_reservetrust_tb__email" class="reservetrust_tb__email"><?php echo $reservetrust_tb->_email->FldCaption() ?></span></td>
<?php } ?>
<?php if ($reservetrust_tb->phoneno->Visible) { // phoneno ?>
		<td><span id="elh_reservetrust_tb_phoneno" class="reservetrust_tb_phoneno"><?php echo $reservetrust_tb->phoneno->FldCaption() ?></span></td>
<?php } ?>
<?php if ($reservetrust_tb->gender->Visible) { // gender ?>
		<td><span id="elh_reservetrust_tb_gender" class="reservetrust_tb_gender"><?php echo $reservetrust_tb->gender->FldCaption() ?></span></td>
<?php } ?>
<?php if ($reservetrust_tb->dob->Visible) { // dob ?>
		<td><span id="elh_reservetrust_tb_dob" class="reservetrust_tb_dob"><?php echo $reservetrust_tb->dob->FldCaption() ?></span></td>
<?php } ?>
<?php if ($reservetrust_tb->employmentstatus->Visible) { // employmentstatus ?>
		<td><span id="elh_reservetrust_tb_employmentstatus" class="reservetrust_tb_employmentstatus"><?php echo $reservetrust_tb->employmentstatus->FldCaption() ?></span></td>
<?php } ?>
<?php if ($reservetrust_tb->maritalstatus->Visible) { // maritalstatus ?>
		<td><span id="elh_reservetrust_tb_maritalstatus" class="reservetrust_tb_maritalstatus"><?php echo $reservetrust_tb->maritalstatus->FldCaption() ?></span></td>
<?php } ?>
<?php if ($reservetrust_tb->datecreated->Visible) { // datecreated ?>
		<td><span id="elh_reservetrust_tb_datecreated" class="reservetrust_tb_datecreated"><?php echo $reservetrust_tb->datecreated->FldCaption() ?></span></td>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$reservetrust_tb_delete->RecCnt = 0;
$i = 0;
while (!$reservetrust_tb_delete->Recordset->EOF) {
	$reservetrust_tb_delete->RecCnt++;
	$reservetrust_tb_delete->RowCnt++;

	// Set row properties
	$reservetrust_tb->ResetAttrs();
	$reservetrust_tb->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$reservetrust_tb_delete->LoadRowValues($reservetrust_tb_delete->Recordset);

	// Render row
	$reservetrust_tb_delete->RenderRow();
?>
	<tr<?php echo $reservetrust_tb->RowAttributes() ?>>
<?php if ($reservetrust_tb->fullname->Visible) { // fullname ?>
		<td<?php echo $reservetrust_tb->fullname->CellAttributes() ?>>
<span id="el<?php echo $reservetrust_tb_delete->RowCnt ?>_reservetrust_tb_fullname" class="control-group reservetrust_tb_fullname">
<span<?php echo $reservetrust_tb->fullname->ViewAttributes() ?>>
<?php echo $reservetrust_tb->fullname->ListViewValue() ?></span>
</span>
</td>
<?php } ?>
<?php if ($reservetrust_tb->_email->Visible) { // email ?>
		<td<?php echo $reservetrust_tb->_email->CellAttributes() ?>>
<span id="el<?php echo $reservetrust_tb_delete->RowCnt ?>_reservetrust_tb__email" class="control-group reservetrust_tb__email">
<span<?php echo $reservetrust_tb->_email->ViewAttributes() ?>>
<?php echo $reservetrust_tb->_email->ListViewValue() ?></span>
</span>
</td>
<?php } ?>
<?php if ($reservetrust_tb->phoneno->Visible) { // phoneno ?>
		<td<?php echo $reservetrust_tb->phoneno->CellAttributes() ?>>
<span id="el<?php echo $reservetrust_tb_delete->RowCnt ?>_reservetrust_tb_phoneno" class="control-group reservetrust_tb_phoneno">
<span<?php echo $reservetrust_tb->phoneno->ViewAttributes() ?>>
<?php echo $reservetrust_tb->phoneno->ListViewValue() ?></span>
</span>
</td>
<?php } ?>
<?php if ($reservetrust_tb->gender->Visible) { // gender ?>
		<td<?php echo $reservetrust_tb->gender->CellAttributes() ?>>
<span id="el<?php echo $reservetrust_tb_delete->RowCnt ?>_reservetrust_tb_gender" class="control-group reservetrust_tb_gender">
<span<?php echo $reservetrust_tb->gender->ViewAttributes() ?>>
<?php echo $reservetrust_tb->gender->ListViewValue() ?></span>
</span>
</td>
<?php } ?>
<?php if ($reservetrust_tb->dob->Visible) { // dob ?>
		<td<?php echo $reservetrust_tb->dob->CellAttributes() ?>>
<span id="el<?php echo $reservetrust_tb_delete->RowCnt ?>_reservetrust_tb_dob" class="control-group reservetrust_tb_dob">
<span<?php echo $reservetrust_tb->dob->ViewAttributes() ?>>
<?php echo $reservetrust_tb->dob->ListViewValue() ?></span>
</span>
</td>
<?php } ?>
<?php if ($reservetrust_tb->employmentstatus->Visible) { // employmentstatus ?>
		<td<?php echo $reservetrust_tb->employmentstatus->CellAttributes() ?>>
<span id="el<?php echo $reservetrust_tb_delete->RowCnt ?>_reservetrust_tb_employmentstatus" class="control-group reservetrust_tb_employmentstatus">
<span<?php echo $reservetrust_tb->employmentstatus->ViewAttributes() ?>>
<?php echo $reservetrust_tb->employmentstatus->ListViewValue() ?></span>
</span>
</td>
<?php } ?>
<?php if ($reservetrust_tb->maritalstatus->Visible) { // maritalstatus ?>
		<td<?php echo $reservetrust_tb->maritalstatus->CellAttributes() ?>>
<span id="el<?php echo $reservetrust_tb_delete->RowCnt ?>_reservetrust_tb_maritalstatus" class="control-group reservetrust_tb_maritalstatus">
<span<?php echo $reservetrust_tb->maritalstatus->ViewAttributes() ?>>
<?php echo $reservetrust_tb->maritalstatus->ListViewValue() ?></span>
</span>
</td>
<?php } ?>
<?php if ($reservetrust_tb->datecreated->Visible) { // datecreated ?>
		<td<?php echo $reservetrust_tb->datecreated->CellAttributes() ?>>
<span id="el<?php echo $reservetrust_tb_delete->RowCnt ?>_reservetrust_tb_datecreated" class="control-group reservetrust_tb_datecreated">
<span<?php echo $reservetrust_tb->datecreated->ViewAttributes() ?>>
<?php echo $reservetrust_tb->datecreated->ListViewValue() ?></span>
</span>
</td>
<?php } ?>
	</tr>
<?php
	$reservetrust_tb_delete->Recordset->MoveNext();
}
$reservetrust_tb_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
freservetrust_tbdelete.Init();
</script>
<?php
$reservetrust_tb_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$reservetrust_tb_delete->Page_Terminate();
?>
