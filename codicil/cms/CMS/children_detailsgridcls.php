...
$children_details_grid = NULL; // Initialize page object first

class cchildren_details_grid extends cchildren_details {
	...
	// Page object name
	var $PageObjName = 'children_details_grid';

	// Grid form hidden field names
	var $FormName = 'fchildren_detailsgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["children_details"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_gender") && $objForm->HasValue("o_gender") && $this->gender->CurrentValue <> $this->gender->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_dob") && $objForm->HasValue("o_dob") && $this->dob->CurrentValue <> $this->dob->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_age") && $objForm->HasValue("o_age") && $this->age->CurrentValue <> $this->age->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_title") && $objForm->HasValue("o_title") && $this->title->CurrentValue <> $this->title->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_guardianname") && $objForm->HasValue("o_guardianname") && $this->guardianname->CurrentValue <> $this->guardianname->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_rtionship") && $objForm->HasValue("o_rtionship") && $this->rtionship->CurrentValue <> $this->rtionship->OldValue)
			return FALSE;
	...
	function LoadFormValues() {
		...
		$this->dob->setOldValue($objForm->GetValue("o_dob"));
		if (!$this->age->FldIsDetailKey) {
			$this->age->setFormValue($objForm->GetValue("x_age"));
		}
		$this->age->setOldValue($objForm->GetValue("o_age"));
		...
		$this->guardianname->setOldValue($objForm->GetValue("o_guardianname"));
		if (!$this->rtionship->FldIsDetailKey) {
			$this->rtionship->setFormValue($objForm->GetValue("x_rtionship"));
		}
		$this->rtionship->setOldValue($objForm->GetValue("o_rtionship"));
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "spouse_tb") {
			$this->uid->Visible = FALSE;
			if ($GLOBALS["spouse_tb"]->EventCancelled) $this->EventCancelled = TRUE;
		}
