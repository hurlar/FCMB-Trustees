...
class cpersonal_info_view extends cpersonal_info {
	...
	function SetupOtherOptions() {
		...
		// Detail table 'nextofkin'
		$body = $Language->TablePhrase("nextofkin", "TblCaption");
		$body = "<a class=\"ewAction ewDetailList\" href=\"" . ew_HtmlEncode("nextofkinlist.php?" . EW_TABLE_SHOW_MASTER . "=personal_info&uid=" . strval($this->uid->CurrentValue) . "") . "\">" . $body . "</a>";
		$item = &$option->Add("detail_nextofkin");
		$item->Body = $body;
		$item->Visible = $Security->IsLoggedIn();
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
			$DetailTableLink .= "nextofkin";
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("nextofkin", $DetailTblVar)) {
				if (!isset($GLOBALS["nextofkin_grid"]))
					$GLOBALS["nextofkin_grid"] = new cnextofkin_grid;
				if ($GLOBALS["nextofkin_grid"]->DetailView) {
					$GLOBALS["nextofkin_grid"]->CurrentMode = "view";

					// Save current master table to detail table
					$GLOBALS["nextofkin_grid"]->setCurrentMasterTable($this->TableVar);
					$GLOBALS["nextofkin_grid"]->setStartRecordNumber(1);
					$GLOBALS["nextofkin_grid"]->uid->FldIsDetailKey = TRUE;
					$GLOBALS["nextofkin_grid"]->uid->CurrentValue = $this->uid->CurrentValue;
					$GLOBALS["nextofkin_grid"]->uid->setSessionValue($GLOBALS["nextofkin_grid"]->uid->CurrentValue);
				}
			}
...
<?php } ?>
<?php
	if (in_array("nextofkin", explode(",", $personal_info->getCurrentDetailTable())) && $nextofkin->DetailView) {
?>
<?php include_once "nextofkingrid.php" ?>
<?php } ?>
<?php
