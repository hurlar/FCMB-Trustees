...
$witness_tb_grid = NULL; // Initialize page object first

class cwitness_tb_grid extends cwitness_tb {
	...
	// Page object name
	var $PageObjName = 'witness_tb_grid';

	// Grid form hidden field names
	var $FormName = 'fwitness_tbgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["witness_tb"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_uid") && $objForm->HasValue("o_uid") && $this->uid->CurrentValue <> $this->uid->OldValue)
			return FALSE;
	...
	function LoadFormValues() {
		...
		$this->uid->setOldValue($objForm->GetValue("o_uid"));
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// rtionship
			$this->rtionship->SetDbValueDef($rsnew, $this->rtionship->CurrentValue, "", $this->rtionship->ReadOnly);
