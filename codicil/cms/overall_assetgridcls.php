...
$overall_asset_grid = NULL; // Initialize page object first

class coverall_asset_grid extends coverall_asset {
	...
	// Page object name
	var $PageObjName = 'overall_asset_grid';

	// Grid form hidden field names
	var $FormName = 'foverall_assetgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["overall_asset"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_beneficiaryid") && $objForm->HasValue("o_beneficiaryid") && $this->beneficiaryid->CurrentValue <> $this->beneficiaryid->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_propertyid") && $objForm->HasValue("o_propertyid") && $this->propertyid->CurrentValue <> $this->propertyid->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_property_type") && $objForm->HasValue("o_property_type") && $this->property_type->CurrentValue <> $this->property_type->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_percentage") && $objForm->HasValue("o_percentage") && $this->percentage->CurrentValue <> $this->percentage->OldValue)
			return FALSE;
	...
	function LoadFormValues() {
		...
		$this->beneficiaryid->setOldValue($objForm->GetValue("o_beneficiaryid"));
		if (!$this->propertyid->FldIsDetailKey) {
			$this->propertyid->setFormValue($objForm->GetValue("x_propertyid"));
		}
		$this->propertyid->setOldValue($objForm->GetValue("o_propertyid"));
		...
		$this->percentage->setOldValue($objForm->GetValue("o_percentage"));
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "assets_tb") {
			...
			if ($GLOBALS["assets_tb"]->EventCancelled) $this->EventCancelled = TRUE;
		}
