...
$products_img2_blobview = NULL; // Initialize page object first

class cproducts_img2_blobview extends cproducts {
	...
	// Page object name
	var $PageObjName = 'products_img2_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('products', 'x_img2');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('img2');
				...
				if (trim(strval($this->Recordset->fields('img2'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('img2') . "\"");
				}
...
// Create page object
if (!isset($products_img2_blobview)) $products_img2_blobview = new cproducts_img2_blobview();

// Page init
$products_img2_blobview->Page_Init();

// Page main
$products_img2_blobview->Page_Main();
?>
<?php
$products_img2_blobview->Page_Terminate();
?>
