...
$privatetrust_tb_list = NULL; // Initialize page object first

class cprivatetrust_tb_list extends cprivatetrust_tb {
	...
	// Page object name
	var $PageObjName = 'privatetrust_tb_list';

	// Grid form hidden field names
	var $FormName = 'fprivatetrust_tblist';
	...
	function __construct() {
		...
		$this->AddUrl = "privatetrust_tbadd.php?" . EW_TABLE_SHOW_DETAIL . "=";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "privatetrust_tbdelete.php";
		$this->MultiUpdateUrl = "privatetrust_tbupdate.php";
	...
	function RenderListOptions() {
		...
		if ($Security->IsLoggedIn()) {
			...
			$body = "<a class=\"btn btn-small ewRowLink ewDetailList\" data-action=\"list\" href=\"" . ew_HtmlEncode("personal_infolist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . strval($this->uid->CurrentValue) . "") . "\">" . $body . "</a>";
			$links = "";
		...
		if ($Security->IsLoggedIn()) {
			...
			$body = "<a class=\"btn btn-small ewRowLink ewDetailList\" data-action=\"list\" href=\"" . ew_HtmlEncode("spouse_tblist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . strval($this->uid->CurrentValue) . "") . "\">" . $body . "</a>";
			$links = "";
		...
		if ($Security->IsLoggedIn()) {
			...
			$body = "<a class=\"btn btn-small ewRowLink ewDetailList\" data-action=\"list\" href=\"" . ew_HtmlEncode("divorce_tblist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . strval($this->uid->CurrentValue) . "") . "\">" . $body . "</a>";
			$links = "";
		...
		if ($Security->IsLoggedIn()) {
			...
			$body = "<a class=\"btn btn-small ewRowLink ewDetailList\" data-action=\"list\" href=\"" . ew_HtmlEncode("children_detailslist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . strval($this->uid->CurrentValue) . "") . "\">" . $body . "</a>";
			$links = "";
		...
		if ($Security->IsLoggedIn()) {
			...
			$body = "<a class=\"btn btn-small ewRowLink ewDetailList\" data-action=\"list\" href=\"" . ew_HtmlEncode("beneficiary_dumplist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . strval($this->uid->CurrentValue) . "") . "\">" . $body . "</a>";
			$links = "";
		...
		if ($Security->IsLoggedIn()) {
			...
			$body = "<a class=\"btn btn-small ewRowLink ewDetailList\" data-action=\"list\" href=\"" . ew_HtmlEncode("alt_beneficiarylist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . strval($this->uid->CurrentValue) . "") . "\">" . $body . "</a>";
			$links = "";
		...
		if ($Security->IsLoggedIn()) {
			...
			$body = "<a class=\"btn btn-small ewRowLink ewDetailList\" data-action=\"list\" href=\"" . ew_HtmlEncode("assets_tblist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . strval($this->uid->CurrentValue) . "") . "\">" . $body . "</a>";
			$links = "";
		...
		if ($Security->IsLoggedIn()) {
			...
			$body = "<a class=\"btn btn-small ewRowLink ewDetailList\" data-action=\"list\" href=\"" . ew_HtmlEncode("overall_assetlist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . strval($this->uid->CurrentValue) . "") . "\">" . $body . "</a>";
			$links = "";
		...
		if ($Security->IsLoggedIn()) {
			...
			$body = "<a class=\"btn btn-small ewRowLink ewDetailList\" data-action=\"list\" href=\"" . ew_HtmlEncode("executor_tblist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . strval($this->uid->CurrentValue) . "") . "\">" . $body . "</a>";
			$links = "";
		...
		if ($Security->IsLoggedIn()) {
			...
			$body = "<a class=\"btn btn-small ewRowLink ewDetailList\" data-action=\"list\" href=\"" . ew_HtmlEncode("trustee_tblist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . strval($this->uid->CurrentValue) . "") . "\">" . $body . "</a>";
			$links = "";
		...
		if ($Security->IsLoggedIn()) {
			...
			$body = "<a class=\"btn btn-small ewRowLink ewDetailList\" data-action=\"list\" href=\"" . ew_HtmlEncode("witness_tblist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . strval($this->uid->CurrentValue) . "") . "\">" . $body . "</a>";
			$links = "";
		...
		if ($Security->IsLoggedIn()) {
			...
			$body = "<a class=\"btn btn-small ewRowLink ewDetailList\" data-action=\"list\" href=\"" . ew_HtmlEncode("addinfo_tblist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . strval($this->uid->CurrentValue) . "") . "\">" . $body . "</a>";
			$links = "";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewAction ewMultiDelete\" href=\"\" onclick=\"ew_SubmitSelected(document.fprivatetrust_tblist, '" . $this->MultiDeleteUrl . "', ewLanguage.Phrase('DeleteMultiConfirmMsg'));return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = ($Security->IsLoggedIn());
	...
	function RenderOtherOptions() {
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->CustomActions as $action => $name) {
				...
				$item->Body = "<a class=\"ewAction ewCustomAction\" href=\"\" onclick=\"ew_SubmitSelected(document.fprivatetrust_tblist, '" . ew_CurrentUrl() . "', null, '" . $action . "');return false;\">" . $name . "</a>";
			}
	...
	function RenderListOptionsExt() {
		...
		if ($Security->IsLoggedIn()) {			
			...
			$links .= $link;
			$detaillnk = ew_JsEncode3("personal_infolist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . urlencode(strval($this->uid->CurrentValue)) . "");
		...
		if ($Security->IsLoggedIn()) {			
			...
			$links .= $link;
			$detaillnk = ew_JsEncode3("spouse_tblist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . urlencode(strval($this->uid->CurrentValue)) . "");
		...
		if ($Security->IsLoggedIn()) {			
			...
			$links .= $link;
			$detaillnk = ew_JsEncode3("divorce_tblist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . urlencode(strval($this->uid->CurrentValue)) . "");
		...
		if ($Security->IsLoggedIn()) {			
			...
			$links .= $link;
			$detaillnk = ew_JsEncode3("children_detailslist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . urlencode(strval($this->uid->CurrentValue)) . "");
		...
		if ($Security->IsLoggedIn()) {			
			...
			$links .= $link;
			$detaillnk = ew_JsEncode3("beneficiary_dumplist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . urlencode(strval($this->uid->CurrentValue)) . "");
		...
		if ($Security->IsLoggedIn()) {			
			...
			$links .= $link;
			$detaillnk = ew_JsEncode3("alt_beneficiarylist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . urlencode(strval($this->uid->CurrentValue)) . "");
		...
		if ($Security->IsLoggedIn()) {			
			...
			$links .= $link;
			$detaillnk = ew_JsEncode3("assets_tblist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . urlencode(strval($this->uid->CurrentValue)) . "");
		...
		if ($Security->IsLoggedIn()) {			
			...
			$links .= $link;
			$detaillnk = ew_JsEncode3("overall_assetlist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . urlencode(strval($this->uid->CurrentValue)) . "");
		...
		if ($Security->IsLoggedIn()) {			
			...
			$links .= $link;
			$detaillnk = ew_JsEncode3("executor_tblist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . urlencode(strval($this->uid->CurrentValue)) . "");
		...
		if ($Security->IsLoggedIn()) {			
			...
			$links .= $link;
			$detaillnk = ew_JsEncode3("trustee_tblist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . urlencode(strval($this->uid->CurrentValue)) . "");
		...
		$sSqlWrk = "`id`=" . ew_AdjustSql($this->uid->CurrentValue) . "";
		...
		if ($Security->IsLoggedIn()) {			
			...
			$links .= $link;
			$detaillnk = ew_JsEncode3("witness_tblist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . urlencode(strval($this->uid->CurrentValue)) . "");
		...
		if ($Security->IsLoggedIn()) {			
			...
			$links .= $link;
			$detaillnk = ew_JsEncode3("addinfo_tblist.php?" . EW_TABLE_SHOW_MASTER . "=privatetrust_tb&uid=" . urlencode(strval($this->uid->CurrentValue)) . "");
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (!ew_Empty($this->uid->CurrentValue)) {
				$this->fullname->HrefValue = "../portal/adminprivatetrust-preview.php?a=" . ((!empty($this->uid->ViewValue)) ? $this->uid->ViewValue : $this->uid->CurrentValue); // Add prefix/suffix
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_privatetrust_tb\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_privatetrust_tb',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fprivatetrust_tblist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($privatetrust_tb_list)) $privatetrust_tb_list = new cprivatetrust_tb_list();

// Page init
$privatetrust_tb_list->Page_Init();

// Page main
$privatetrust_tb_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$privatetrust_tb_list->Page_Render();
?>
...
// Page object
var privatetrust_tb_list = new ew_Page("privatetrust_tb_list");
privatetrust_tb_list.PageID = "list"; // Page ID
var EW_PAGE_ID = privatetrust_tb_list.PageID; // For backward compatibility

// Form object
var fprivatetrust_tblist = new ew_Form("fprivatetrust_tblist");
fprivatetrust_tblist.FormKeyCountName = '<?php echo $privatetrust_tb_list->FormKeyCountName ?>';

// Form_CustomValidate event
fprivatetrust_tblist.Form_CustomValidate = 
...
fprivatetrust_tblist.ValidateRequired = true;
<?php } else { ?>
fprivatetrust_tblist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
// Form object for search

var fprivatetrust_tblistsrch = new ew_Form("fprivatetrust_tblistsrch");
</script>
<style type="text/css">
...
<?php } ?>
<?php if ($privatetrust_tb_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $privatetrust_tb_list->ExportOptions->Render("body") ?></div>
<?php } ?>
<?php
	$bSelectLimit = EW_SELECT_LIMIT;
	if ($bSelectLimit) {
		$privatetrust_tb_list->TotalRecs = $privatetrust_tb->SelectRecordCount();
	} else {
		if ($privatetrust_tb_list->Recordset = $privatetrust_tb_list->LoadRecordset())
			$privatetrust_tb_list->TotalRecs = $privatetrust_tb_list->Recordset->RecordCount();
	}
	$privatetrust_tb_list->StartRec = 1;
	if ($privatetrust_tb_list->DisplayRecs <= 0 || ($privatetrust_tb->Export <> "" && $privatetrust_tb->ExportAll)) // Display all records
		$privatetrust_tb_list->DisplayRecs = $privatetrust_tb_list->TotalRecs;
	if (!($privatetrust_tb->Export <> "" && $privatetrust_tb->ExportAll))
		$privatetrust_tb_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$privatetrust_tb_list->Recordset = $privatetrust_tb_list->LoadRecordset($privatetrust_tb_list->StartRec-1, $privatetrust_tb_list->DisplayRecs);
$privatetrust_tb_list->RenderOtherOptions();
?>
<?php if ($Security->IsLoggedIn()) { ?>
<?php if ($privatetrust_tb->Export == "" && $privatetrust_tb->CurrentAction == "") { ?>
<form name="fprivatetrust_tblistsrch" id="fprivatetrust_tblistsrch" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>">
<table class="ewSearchTable"><tr><td>
<div class="accordion" id="fprivatetrust_tblistsrch_SearchGroup">
	<div class="accordion-group">
		<div class="accordion-heading">
<a class="accordion-toggle" data-toggle="collapse" data-parent="#fprivatetrust_tblistsrch_SearchGroup" href="#fprivatetrust_tblistsrch_SearchBody"><?php echo $Language->Phrase("Search") ?></a>
		</div>
		<div id="fprivatetrust_tblistsrch_SearchBody" class="accordion-body collapse in">
			<div class="accordion-inner">
<div id="fprivatetrust_tblistsrch_SearchPanel">
...
<div id="xsr_1" class="ewRow">
	...
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="input-large" value="<?php echo ew_HtmlEncode($privatetrust_tb_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo $Language->Phrase("Search") ?>">
	<button class="btn btn-primary ewButton" name="btnsubmit" id="btnsubmit" type="submit"><?php echo $Language->Phrase("QuickSearchBtn") ?></button>
	</div>
	</div>
	<div class="btn-group ewButtonGroup">
	<a class="btn ewShowAll" href="<?php echo $privatetrust_tb_list->PageUrl() ?>cmd=reset"><?php echo $Language->Phrase("ShowAll") ?></a>
</div>
<div id="xsr_2" class="ewRow">
	<label class="inline radio ewRadio" style="white-space: nowrap;"><input type="radio" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="="<?php if ($privatetrust_tb_list->BasicSearch->getType() == "=") { ?> checked="checked"<?php } ?>><?php echo $Language->Phrase("ExactPhrase") ?></label>
	<label class="inline radio ewRadio" style="white-space: nowrap;"><input type="radio" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="AND"<?php if ($privatetrust_tb_list->BasicSearch->getType() == "AND") { ?> checked="checked"<?php } ?>><?php echo $Language->Phrase("AllWord") ?></label>
	<label class="inline radio ewRadio" style="white-space: nowrap;"><input type="radio" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="OR"<?php if ($privatetrust_tb_list->BasicSearch->getType() == "OR") { ?> checked="checked"<?php } ?>><?php echo $Language->Phrase("AnyWord") ?></label>
</div>
</div>
</div>
			</div>
...
<?php } ?>
<?php } ?>
<?php $privatetrust_tb_list->ShowPageHeader(); ?>
<?php
$privatetrust_tb_list->ShowMessage();
?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<form name="fprivatetrust_tblist" id="fprivatetrust_tblist" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="privatetrust_tb">
<div id="gmp_privatetrust_tb" class="ewGridMiddlePanel">
<?php if ($privatetrust_tb_list->TotalRecs > 0) { ?>
<table id="tbl_privatetrust_tblist" class="ewTable ewTableSeparate">
<?php echo $privatetrust_tb->TableCustomInnerHtml ?>
<thead><!-- Table header -->
	<tr class="ewTableHeader">
<?php

// Render list options
$privatetrust_tb_list->RenderListOptions();

// Render list options (header, left)
$privatetrust_tb_list->ListOptions->Render("header", "left");
?>
<?php if ($privatetrust_tb->willtype->Visible) { // willtype ?>
	<?php if ($privatetrust_tb->SortUrl($privatetrust_tb->willtype) == "") { ?>
		<td><div id="elh_privatetrust_tb_willtype" class="privatetrust_tb_willtype"><div class="ewTableHeaderCaption"><?php echo $privatetrust_tb->willtype->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $privatetrust_tb->SortUrl($privatetrust_tb->willtype) ?>',2);"><div id="elh_privatetrust_tb_willtype" class="privatetrust_tb_willtype">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $privatetrust_tb->willtype->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($privatetrust_tb->willtype->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($privatetrust_tb->willtype->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($privatetrust_tb->fullname->Visible) { // fullname ?>
	<?php if ($privatetrust_tb->SortUrl($privatetrust_tb->fullname) == "") { ?>
		<td><div id="elh_privatetrust_tb_fullname" class="privatetrust_tb_fullname"><div class="ewTableHeaderCaption"><?php echo $privatetrust_tb->fullname->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $privatetrust_tb->SortUrl($privatetrust_tb->fullname) ?>',2);"><div id="elh_privatetrust_tb_fullname" class="privatetrust_tb_fullname">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $privatetrust_tb->fullname->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($privatetrust_tb->fullname->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($privatetrust_tb->fullname->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($privatetrust_tb->_email->Visible) { // email ?>
	<?php if ($privatetrust_tb->SortUrl($privatetrust_tb->_email) == "") { ?>
		<td><div id="elh_privatetrust_tb__email" class="privatetrust_tb__email"><div class="ewTableHeaderCaption"><?php echo $privatetrust_tb->_email->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $privatetrust_tb->SortUrl($privatetrust_tb->_email) ?>',2);"><div id="elh_privatetrust_tb__email" class="privatetrust_tb__email">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $privatetrust_tb->_email->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($privatetrust_tb->_email->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($privatetrust_tb->_email->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($privatetrust_tb->phoneno->Visible) { // phoneno ?>
	<?php if ($privatetrust_tb->SortUrl($privatetrust_tb->phoneno) == "") { ?>
		<td><div id="elh_privatetrust_tb_phoneno" class="privatetrust_tb_phoneno"><div class="ewTableHeaderCaption"><?php echo $privatetrust_tb->phoneno->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $privatetrust_tb->SortUrl($privatetrust_tb->phoneno) ?>',2);"><div id="elh_privatetrust_tb_phoneno" class="privatetrust_tb_phoneno">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $privatetrust_tb->phoneno->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($privatetrust_tb->phoneno->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($privatetrust_tb->phoneno->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($privatetrust_tb->aphoneno->Visible) { // aphoneno ?>
	<?php if ($privatetrust_tb->SortUrl($privatetrust_tb->aphoneno) == "") { ?>
		<td><div id="elh_privatetrust_tb_aphoneno" class="privatetrust_tb_aphoneno"><div class="ewTableHeaderCaption"><?php echo $privatetrust_tb->aphoneno->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $privatetrust_tb->SortUrl($privatetrust_tb->aphoneno) ?>',2);"><div id="elh_privatetrust_tb_aphoneno" class="privatetrust_tb_aphoneno">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $privatetrust_tb->aphoneno->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($privatetrust_tb->aphoneno->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($privatetrust_tb->aphoneno->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($privatetrust_tb->gender->Visible) { // gender ?>
	<?php if ($privatetrust_tb->SortUrl($privatetrust_tb->gender) == "") { ?>
		<td><div id="elh_privatetrust_tb_gender" class="privatetrust_tb_gender"><div class="ewTableHeaderCaption"><?php echo $privatetrust_tb->gender->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $privatetrust_tb->SortUrl($privatetrust_tb->gender) ?>',2);"><div id="elh_privatetrust_tb_gender" class="privatetrust_tb_gender">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $privatetrust_tb->gender->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($privatetrust_tb->gender->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($privatetrust_tb->gender->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($privatetrust_tb->dob->Visible) { // dob ?>
	<?php if ($privatetrust_tb->SortUrl($privatetrust_tb->dob) == "") { ?>
		<td><div id="elh_privatetrust_tb_dob" class="privatetrust_tb_dob"><div class="ewTableHeaderCaption"><?php echo $privatetrust_tb->dob->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $privatetrust_tb->SortUrl($privatetrust_tb->dob) ?>',2);"><div id="elh_privatetrust_tb_dob" class="privatetrust_tb_dob">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $privatetrust_tb->dob->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($privatetrust_tb->dob->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($privatetrust_tb->dob->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($privatetrust_tb->state->Visible) { // state ?>
	<?php if ($privatetrust_tb->SortUrl($privatetrust_tb->state) == "") { ?>
		<td><div id="elh_privatetrust_tb_state" class="privatetrust_tb_state"><div class="ewTableHeaderCaption"><?php echo $privatetrust_tb->state->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $privatetrust_tb->SortUrl($privatetrust_tb->state) ?>',2);"><div id="elh_privatetrust_tb_state" class="privatetrust_tb_state">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $privatetrust_tb->state->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($privatetrust_tb->state->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($privatetrust_tb->state->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($privatetrust_tb->nationality->Visible) { // nationality ?>
	<?php if ($privatetrust_tb->SortUrl($privatetrust_tb->nationality) == "") { ?>
		<td><div id="elh_privatetrust_tb_nationality" class="privatetrust_tb_nationality"><div class="ewTableHeaderCaption"><?php echo $privatetrust_tb->nationality->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $privatetrust_tb->SortUrl($privatetrust_tb->nationality) ?>',2);"><div id="elh_privatetrust_tb_nationality" class="privatetrust_tb_nationality">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $privatetrust_tb->nationality->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($privatetrust_tb->nationality->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($privatetrust_tb->nationality->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($privatetrust_tb->employmentstatus->Visible) { // employmentstatus ?>
	<?php if ($privatetrust_tb->SortUrl($privatetrust_tb->employmentstatus) == "") { ?>
		<td><div id="elh_privatetrust_tb_employmentstatus" class="privatetrust_tb_employmentstatus"><div class="ewTableHeaderCaption"><?php echo $privatetrust_tb->employmentstatus->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $privatetrust_tb->SortUrl($privatetrust_tb->employmentstatus) ?>',2);"><div id="elh_privatetrust_tb_employmentstatus" class="privatetrust_tb_employmentstatus">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $privatetrust_tb->employmentstatus->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($privatetrust_tb->employmentstatus->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($privatetrust_tb->employmentstatus->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($privatetrust_tb->maritalstatus->Visible) { // maritalstatus ?>
	<?php if ($privatetrust_tb->SortUrl($privatetrust_tb->maritalstatus) == "") { ?>
		<td><div id="elh_privatetrust_tb_maritalstatus" class="privatetrust_tb_maritalstatus"><div class="ewTableHeaderCaption"><?php echo $privatetrust_tb->maritalstatus->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $privatetrust_tb->SortUrl($privatetrust_tb->maritalstatus) ?>',2);"><div id="elh_privatetrust_tb_maritalstatus" class="privatetrust_tb_maritalstatus">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $privatetrust_tb->maritalstatus->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($privatetrust_tb->maritalstatus->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($privatetrust_tb->maritalstatus->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($privatetrust_tb->datecreated->Visible) { // datecreated ?>
	<?php if ($privatetrust_tb->SortUrl($privatetrust_tb->datecreated) == "") { ?>
		<td><div id="elh_privatetrust_tb_datecreated" class="privatetrust_tb_datecreated"><div class="ewTableHeaderCaption"><?php echo $privatetrust_tb->datecreated->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $privatetrust_tb->SortUrl($privatetrust_tb->datecreated) ?>',2);"><div id="elh_privatetrust_tb_datecreated" class="privatetrust_tb_datecreated">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $privatetrust_tb->datecreated->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($privatetrust_tb->datecreated->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($privatetrust_tb->datecreated->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php
...
$privatetrust_tb_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($privatetrust_tb->ExportAll && $privatetrust_tb->Export <> "") {
	$privatetrust_tb_list->StopRec = $privatetrust_tb_list->TotalRecs;
} else {

	// Set the last record to display
	if ($privatetrust_tb_list->TotalRecs > $privatetrust_tb_list->StartRec + $privatetrust_tb_list->DisplayRecs - 1)
		$privatetrust_tb_list->StopRec = $privatetrust_tb_list->StartRec + $privatetrust_tb_list->DisplayRecs - 1;
	else
		$privatetrust_tb_list->StopRec = $privatetrust_tb_list->TotalRecs;
}
$privatetrust_tb_list->RecCnt = $privatetrust_tb_list->StartRec - 1;
if ($privatetrust_tb_list->Recordset && !$privatetrust_tb_list->Recordset->EOF) {
	$privatetrust_tb_list->Recordset->MoveFirst();
	if (!$bSelectLimit && $privatetrust_tb_list->StartRec > 1)
		$privatetrust_tb_list->Recordset->Move($privatetrust_tb_list->StartRec - 1);
} elseif (!$privatetrust_tb->AllowAddDeleteRow && $privatetrust_tb_list->StopRec == 0) {
	$privatetrust_tb_list->StopRec = $privatetrust_tb->GridAddRowCount;
}

// Initialize aggregate
$privatetrust_tb->RowType = EW_ROWTYPE_AGGREGATEINIT;
$privatetrust_tb->ResetAttrs();
$privatetrust_tb_list->RenderRow();
while ($privatetrust_tb_list->RecCnt < $privatetrust_tb_list->StopRec) {
	$privatetrust_tb_list->RecCnt++;
	if (intval($privatetrust_tb_list->RecCnt) >= intval($privatetrust_tb_list->StartRec)) {
		$privatetrust_tb_list->RowCnt++;

		// Set up key count
		$privatetrust_tb_list->KeyCount = $privatetrust_tb_list->RowIndex;

		// Init row class and style
		$privatetrust_tb->ResetAttrs();
		$privatetrust_tb->CssClass = "";
		if ($privatetrust_tb->CurrentAction == "gridadd") {
		} else {
			$privatetrust_tb_list->LoadRowValues($privatetrust_tb_list->Recordset); // Load row values
		}
		$privatetrust_tb->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$privatetrust_tb->RowAttrs = array_merge($privatetrust_tb->RowAttrs, array('data-rowindex'=>$privatetrust_tb_list->RowCnt, 'id'=>'r' . $privatetrust_tb_list->RowCnt . '_privatetrust_tb', 'data-rowtype'=>$privatetrust_tb->RowType));
		...
		$privatetrust_tb_list->RenderListOptions();
?>
	<tr<?php echo $privatetrust_tb->RowAttributes() ?>>
<?php

// Render list options (body, left)
$privatetrust_tb_list->ListOptions->Render("body", "left", $privatetrust_tb_list->RowCnt);
?>
	<?php if ($privatetrust_tb->willtype->Visible) { // willtype ?>
...
<a id="<?php echo $privatetrust_tb_list->PageObjName . "_row_" . $privatetrust_tb_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($privatetrust_tb->fullname->Visible) { // fullname ?>
...
$privatetrust_tb_list->ListOptions->Render("body", "right", $privatetrust_tb_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($privatetrust_tb->CurrentAction <> "gridadd")
		$privatetrust_tb_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($privatetrust_tb->CurrentAction == "") { ?>
...
// Close recordset
if ($privatetrust_tb_list->Recordset)
	$privatetrust_tb_list->Recordset->Close();
?>
<?php if ($privatetrust_tb->Export == "") { ?>
<div class="ewGridLowerPanel">
<?php if ($privatetrust_tb->CurrentAction <> "gridadd" && $privatetrust_tb->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($privatetrust_tb_list->Pager)) $privatetrust_tb_list->Pager = new cNumericPager($privatetrust_tb_list->StartRec, $privatetrust_tb_list->DisplayRecs, $privatetrust_tb_list->TotalRecs, $privatetrust_tb_list->RecRange) ?>
<?php if ($privatetrust_tb_list->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($privatetrust_tb_list->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $privatetrust_tb_list->PageUrl() ?>start=<?php echo $privatetrust_tb_list->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($privatetrust_tb_list->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $privatetrust_tb_list->PageUrl() ?>start=<?php echo $privatetrust_tb_list->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($privatetrust_tb_list->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $privatetrust_tb_list->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($privatetrust_tb_list->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $privatetrust_tb_list->PageUrl() ?>start=<?php echo $privatetrust_tb_list->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($privatetrust_tb_list->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $privatetrust_tb_list->PageUrl() ?>start=<?php echo $privatetrust_tb_list->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
<td>
	<?php if ($privatetrust_tb_list->Pager->ButtonCount > 0) { ?>&nbsp;&nbsp;&nbsp;&nbsp;<?php } ?>
	<?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $privatetrust_tb_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $privatetrust_tb_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $privatetrust_tb_list->Pager->RecordCount ?>
</td>
</tr></tbody></table>
<?php } else { ?>
	<?php if ($privatetrust_tb_list->SearchWhere == "0=101") { ?>
...
<option value="25"<?php if ($privatetrust_tb_list->DisplayRecs == 25) { ?> selected="selected"<?php } ?>>25</option>
<option value="50"<?php if ($privatetrust_tb_list->DisplayRecs == 50) { ?> selected="selected"<?php } ?>>50</option>
<option value="100"<?php if ($privatetrust_tb_list->DisplayRecs == 100) { ?> selected="selected"<?php } ?>>100</option>
<option value="200"<?php if ($privatetrust_tb_list->DisplayRecs == 200) { ?> selected="selected"<?php } ?>>200</option>
<option value="500"<?php if ($privatetrust_tb_list->DisplayRecs == 500) { ?> selected="selected"<?php } ?>>500</option>
<option value="ALL"<?php if ($privatetrust_tb->getRecordsPerPage() == -1) { ?> selected="selected"<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</td>
<?php } ?>
</tr></table>
...
<?php
	foreach ($privatetrust_tb_list->OtherOptions as &$option)
...
<?php if ($privatetrust_tb->Export == "") { ?>
<script type="text/javascript">
fprivatetrust_tblistsrch.Init();
fprivatetrust_tblist.Init();
...
<?php } ?>
<?php
$privatetrust_tb_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($privatetrust_tb->Export == "") { ?>
...
<?php
$privatetrust_tb_list->Page_Terminate();
?>
