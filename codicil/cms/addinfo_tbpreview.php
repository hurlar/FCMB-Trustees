...
$addinfo_tb_preview = NULL; // Initialize page object first

class caddinfo_tb_preview extends caddinfo_tb {
	...
	// Page object name
	var $PageObjName = 'addinfo_tb_preview';
...
// Create page object
if (!isset($addinfo_tb_preview)) $addinfo_tb_preview = new caddinfo_tb_preview();

// Page init
$addinfo_tb_preview->Page_Init();

// Page main
$addinfo_tb_preview->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$addinfo_tb_preview->Page_Render();
?>
<?php $addinfo_tb_preview->ShowPageHeader(); ?>
<?php if ($addinfo_tb_preview->TotalRecs > 0) { ?>
...
<?php if ($addinfo_tb->id->Visible) { // id ?>
			<td><?php echo $addinfo_tb->id->FldCaption() ?></td>
<?php } ?>
<?php if ($addinfo_tb->uid->Visible) { // uid ?>
			<td><?php echo $addinfo_tb->uid->FldCaption() ?></td>
<?php } ?>
<?php if ($addinfo_tb->addinfo->Visible) { // addinfo ?>
			<td><?php echo $addinfo_tb->addinfo->FldCaption() ?></td>
<?php } ?>
<?php if ($addinfo_tb->datecreated->Visible) { // datecreated ?>
			<td><?php echo $addinfo_tb->datecreated->FldCaption() ?></td>
<?php } ?>
		</tr>
	</thead>
	<tbody><!-- Table body -->
<?php
$addinfo_tb_preview->RecCount = 0;
$addinfo_tb_preview->RowCnt = 0;
while ($addinfo_tb_preview->Recordset && !$addinfo_tb_preview->Recordset->EOF) {

	// Init row class and style
	$addinfo_tb_preview->RecCount++;
	$addinfo_tb_preview->RowCnt++;
	$addinfo_tb->CssClass = "";
	$addinfo_tb->CssStyle = "";
	$addinfo_tb->LoadListRowValues($addinfo_tb_preview->Recordset);

	// Render row
	$addinfo_tb->RowType = EW_ROWTYPE_PREVIEW; // Preview record
	$addinfo_tb->RenderListRow();
?>
...
<?php
	$addinfo_tb_preview->Recordset->MoveNext();
}
?>
	</tbody>
</table>
...
<?php if ($addinfo_tb_preview->TotalRecs > 0) { ?>
<div class="ewDetailCount">(<?php echo $addinfo_tb_preview->TotalRecs ?>&nbsp;<?php echo $Language->Phrase("Record") ?>)</div>
<?php } ?>
<?php
$addinfo_tb_preview->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php
	if ($addinfo_tb_preview->Recordset)
		$addinfo_tb_preview->Recordset->Close();
} else { ?>
...
?>
<?php
$addinfo_tb_preview->Page_Terminate();
?>
