...
$executor_tb_preview = NULL; // Initialize page object first

class cexecutor_tb_preview extends cexecutor_tb {
	...
	// Page object name
	var $PageObjName = 'executor_tb_preview';
...
// Create page object
if (!isset($executor_tb_preview)) $executor_tb_preview = new cexecutor_tb_preview();

// Page init
$executor_tb_preview->Page_Init();

// Page main
$executor_tb_preview->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$executor_tb_preview->Page_Render();
?>
<?php $executor_tb_preview->ShowPageHeader(); ?>
<?php if ($executor_tb_preview->TotalRecs > 0) { ?>
...
<?php if ($executor_tb->id->Visible) { // id ?>
			<td><?php echo $executor_tb->id->FldCaption() ?></td>
<?php } ?>
<?php if ($executor_tb->title->Visible) { // title ?>
			<td><?php echo $executor_tb->title->FldCaption() ?></td>
<?php } ?>
<?php if ($executor_tb->fullname->Visible) { // fullname ?>
			<td><?php echo $executor_tb->fullname->FldCaption() ?></td>
<?php } ?>
<?php if ($executor_tb->rtionship->Visible) { // rtionship ?>
			<td><?php echo $executor_tb->rtionship->FldCaption() ?></td>
<?php } ?>
<?php if ($executor_tb->_email->Visible) { // email ?>
			<td><?php echo $executor_tb->_email->FldCaption() ?></td>
<?php } ?>
<?php if ($executor_tb->phone->Visible) { // phone ?>
			<td><?php echo $executor_tb->phone->FldCaption() ?></td>
<?php } ?>
<?php if ($executor_tb->city->Visible) { // city ?>
			<td><?php echo $executor_tb->city->FldCaption() ?></td>
<?php } ?>
<?php if ($executor_tb->state->Visible) { // state ?>
			<td><?php echo $executor_tb->state->FldCaption() ?></td>
<?php } ?>
<?php if ($executor_tb->datecreated->Visible) { // datecreated ?>
			<td><?php echo $executor_tb->datecreated->FldCaption() ?></td>
<?php } ?>
		</tr>
	</thead>
	<tbody><!-- Table body -->
<?php
$executor_tb_preview->RecCount = 0;
$executor_tb_preview->RowCnt = 0;
while ($executor_tb_preview->Recordset && !$executor_tb_preview->Recordset->EOF) {

	// Init row class and style
	$executor_tb_preview->RecCount++;
	$executor_tb_preview->RowCnt++;
	$executor_tb->CssClass = "";
	$executor_tb->CssStyle = "";
	$executor_tb->LoadListRowValues($executor_tb_preview->Recordset);

	// Render row
	$executor_tb->RowType = EW_ROWTYPE_PREVIEW; // Preview record
	$executor_tb->RenderListRow();
?>
...
<?php
	$executor_tb_preview->Recordset->MoveNext();
}
?>
	</tbody>
</table>
...
<?php if ($executor_tb_preview->TotalRecs > 0) { ?>
<div class="ewDetailCount">(<?php echo $executor_tb_preview->TotalRecs ?>&nbsp;<?php echo $Language->Phrase("Record") ?>)</div>
<?php } ?>
<?php
$executor_tb_preview->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php
	if ($executor_tb_preview->Recordset)
		$executor_tb_preview->Recordset->Close();
} else { ?>
...
?>
<?php
$executor_tb_preview->Page_Terminate();
?>
