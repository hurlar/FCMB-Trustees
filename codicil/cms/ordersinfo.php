...
class corders extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('orders', 'orders', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// customerid
		$this->customerid = new cField('orders', 'orders', 'x_customerid', 'customerid', '`customerid`', '`customerid`', 3, -1, FALSE, '`customerid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->customerid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['customerid'] = &$this->customerid;

		// ship
		$this->ship = new cField('orders', 'orders', 'x_ship', 'ship', '`ship`', '`ship`', 200, -1, FALSE, '`ship`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ship'] = &$this->ship;

		// total
		$this->total = new cField('orders', 'orders', 'x_total', 'total', '`total`', '`total`', 200, -1, FALSE, '`total`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['total'] = &$this->total;

		// status
		$this->status = new cField('orders', 'orders', 'x_status', 'status', '`status`', '`status`', 202, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->status->FldDataType = EW_DATATYPE_BOOLEAN;
		$this->fields['status'] = &$this->status;

		// date
		$this->date = new cField('orders', 'orders', 'x_date', 'date', '`date`', 'DATE_FORMAT(`date`, \'%d/%m/%y\')', 133, -1, FALSE, '`date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("ordersview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("ordersview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("ordersedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("ordersadd.php", $this->UrlParm($parm));
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->ship->Exportable) $Doc->ExportCaption($this->ship);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->ship->Exportable) $Doc->ExportField($this->ship);
