...
mysqli_select_db($conn, $database_conn);
$query_nextofkin = "SELECT  `id`, `uid` FROM nextofkin WHERE uid = '$uid' "; 
$nextofkin = mysqli_query($conn, $query_nextofkin) or die(mysqli_error($conn));
$totalnextofkin = mysqli_num_rows($nextofkin);
if($totalnextofkin < '1'){
    	header("Location:../simplewill-marital-info.php?a=nextofkin");
	exit();
}
...
if ($totalacl == TRUE) {
	...
	$update1 = "UPDATE simplewill_access_level SET access = '1' WHERE uid = '$uid' "; 
	...
	$insert1 = "INSERT INTO simplewill_access_level (uid, access) VALUES ('$uid', '1')";
