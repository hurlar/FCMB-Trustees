...
mysqli_select_db($conn, $database_conn);
$investmentbeneficiary = "SELECT uid FROM investment_beneficiary_status WHERE uid = '$uid' "; 
...
if ($rowinvestmentbeneficiary == TRUE) {
	$updateinvestmentbeneficiary = "UPDATE investment_beneficiary_status SET status = '$beneficiaries' WHERE uid = '$uid'"; 
	$resultupdateinvestmentbeneficiary = mysqli_query($conn, $updateinvestmentbeneficiary) or die(mysqli_error($conn));
}else{
	$insertinvestmentbeneficiary = "INSERT INTO investment_beneficiary_status (uid, status) VALUES ('$uid', '$beneficiaries') ";
	$resultinsertinvestmentbeneficiary = mysqli_query($conn, $insertinvestmentbeneficiary) or die(mysqli_error($conn));
}

mysqli_select_db($conn, $database_conn);
$query_nextofkin = "SELECT  `id`, `uid` FROM investment_nextofkin WHERE uid = '$uid' "; 
$nextofkin = mysqli_query($conn, $query_nextofkin) or die(mysqli_error($conn));
$totalnextofkin = mysqli_num_rows($nextofkin);
if($totalnextofkin < 1){
    	header("Location:../investment-marital-info.php?a=nextofkin");
	exit();
}
...
$sql = "SELECT uid,access FROM investment_access_level WHERE uid = '$uid' "; 
...
if ($totalacl == TRUE) {
	...
	$update1 = "UPDATE investment_access_level SET access = '1' WHERE uid = '$uid' "; 
	...
	$insert1 = "INSERT INTO investment_access_level (uid, access) VALUES ('$uid', '1')";
