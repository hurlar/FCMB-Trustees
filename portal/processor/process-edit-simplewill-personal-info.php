...
$url1 = '../simplewill-marital-info.php';
$url2 = '../edit-simplewill-personal-info.php?a=error';

    $search  = array('&', '-', ' ', '.');
    $replace = array('');

    if(!ctype_alpha(str_replace($search,$replace,$fname))){
        header("Location: ../edit-simplewill-personal-info.php?a=lettersonly");
        exit();
    }elseif(!ctype_alpha(str_replace($search,$replace,$lname))){
        header("Location: ../edit-simplewill-personal-info.php?a=lettersonly");
        exit();
    }elseif(!ctype_digit($phoneno)){
        header("Location: ../edit-simplewill-personal-info.php?a=numbersonly");
        exit();
    }else{

mysqli_select_db($conn, $database_conn);
$sql="UPDATE personal_info SET salutation = '$salutation', fname = '$fname', mname = '$mname', lname = '$lname', dob = '$dob', gender = '$gender', maidenname = '$mothermaidenname', lga = '$lga', nationality = '$nationality', state = '$state', phone = '$phoneno', aphone = '$aphoneno', msg = '$msg', city = '$city', rstate = '$rstate', employment_status = '$estatus',employer = '$employer',employerphone = '$employerphone',employeraddr = '$employeraddr', datecreated = NOW() WHERE uid = '$uid' ";
