...
$url1 = '../dashboard.php';
$url2 = '../reserve-request-savings.php?a=error';

mysqli_select_db($conn, $database_conn);
$selectinvestment = "SELECT `id`,`uid` FROM `reserve_request_savings` WHERE `uid` = '$investmentsavingsid' ";
...
if($totalselectinvestment == 1){
    ...
    $insertinvestment = "INSERT INTO reserve_request_savings (`uid`, `investment_sum`, `investment_returns`,`principal_fund`,`investment_period`,`additional_investment`,`pay_both_to_beneficiaries`,`pay_returns_only_to_beneficiaries`,`reinvest_entire_principal`,`datecreated`) VALUES ('$investmentsavingsid', '$requestamount', '$investmentreturns','$principal', '$trustperiod', '$additionalinvestment','$returntoBeneficiary', '$paymentofInvestmentreturns', '$paymentofreturns', NOW())";
    
    $resultinsertinvestment = mysqli_query($conn, $insertinvestment) or die(mysqli_error($conn));
    
    if($resultinsertinvestment == TRUE){
       $update = "UPDATE reserve_access_level SET access = '2' WHERE uid = '$investmentsavingsid' "; 
