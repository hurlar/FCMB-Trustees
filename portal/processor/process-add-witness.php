...
$url1 = '../add-witness.php?a=successful';
$url2 = '../add-witness.php?a=error';

    $search  = array('&', '-', ' ', '.');
    $replace = array('');
    if(!ctype_alpha(str_replace($search,$replace,$gname))){
        header("Location: ../add-witness.php?a=lettersonly");
        exit();
    }elseif(!ctype_digit($gphoneno)){
        header("Location: ../add-witness.php?a=numbersonly");
        exit();
    }else{
mysqli_select_db($conn, $database_conn);
$sql="INSERT INTO witness_tb (`uid`, `title`, `fullname`,`rtionship`,`email`,`phone`,`addr`,`city`,`state`,`datecreated`) VALUES ('$uid','$gtitle', '$gname', '$grelationship', '$gemail', '$gphoneno', '$gaddr', '$gcity', '$gstate', NOW())";
