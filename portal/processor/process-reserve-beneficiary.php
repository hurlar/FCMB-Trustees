...
$url1 = '../reserve-marital-info.php?a=successful';
$url2 = '../reserve-marital-info.php?a=error';

mysqli_select_db($conn, $database_conn);
$sql="INSERT INTO reserve_beneficiary (`uid`, `title`, `fullname`,`rtionship`,`dob`,`datecreated`) VALUES ('$beneficiaryid', '$beneficiarytitle', '$beneficiaryname','$beneficiaryrelationship', '$beneficiarydob', NOW())";
...
header("Location: $url2"); 
    ...
    mysqli_select_db($conn, $database_conn);
    $sqlbeneficiarystatus = "SELECT * FROM reserve_beneficiary_status WHERE `uid` = '$beneficiaryid' ";
    ...
    if($totalbeneficiary == 1){
       $updatebeneficiarystatus = "UPDATE reserve_beneficiary_status SET `status` = '$beneficiarystatus' WHERE `uid` = '$beneficiaryid' ";
    $updatebeneficiaryresult = mysqli_query($conn, $updatebeneficiarystatus) or die(mysqli_error($conn));
    header("Location: $url1");
    }else{
    $insertbeneficiarystatus = "INSERT INTO reserve_beneficiary_status (`uid`, `status`) VALUES ('$beneficiaryid', '$beneficiarystatus') ";
