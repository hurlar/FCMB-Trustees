...
$rowwtpname = $_SESSION['rowwtpname'];
$querywill = "SELECT `uid`,`willtype` FROM preview_will WHERE uid = '$userid' AND `willtype` = '$rowwtpname' "; 
$will = mysqli_query($conn, $querywill) or die(mysqli_error($conn));
$row_will = mysqli_fetch_assoc($will);
$willtype = $row_will['willtype'];
if($willtype == 'Premium Will'){
    header("Location: select-will.php");
}elseif($willtype == 'Comprehensive Will'){
    header("Location: select-will.php");
}elseif($willtype == 'Estate Planning Questionnaire Form'){
    header("Location: create-trust.php");
}
...
<form action="processor/process-preview.php" method="POST">            
            <div class="row">
